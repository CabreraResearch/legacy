CREATE OR REPLACE TRIGGER TRG_IMPRT_users AFTER INSERT OR DELETE OR UPDATE OF disabled,namefirst,namelast,password,email,employeeid,navrows,locked,failedlogincount,defaultlanguage,phone,username,defaultlocationid,roleid,userid,deleted ON users@CAFLINK FOR EACH ROW 
                                BEGIN
