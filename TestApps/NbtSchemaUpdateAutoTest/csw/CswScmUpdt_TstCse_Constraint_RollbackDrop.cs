...
namespace ChemSW.Nbt.SchemaUpdaterAutoTest
{
    ...
    public class CswScmUpdt_TstCse_Constraint_RollbackDrop : CswScmUpdt_TstCse
    {
        public CswScmUpdt_TstCse_Constraint_RollbackDrop( )
        ...
        public override void runTest()
        {
            ...
            try
            {
                CswTableUpdate CswTableUpdate = _CswNbtSchemaModTrnsctn.makeCswTableUpdate( "CswScmUpdt_TstCse_Constraint_RollbackDrop_update1", _PrimeKeyTable );
            ...
            _CswNbtSchemaModTrnsctn.removeConstraint( _ForeignKeyTable, _ForeignKeyTableFkCol, _PrimeKeyTable, _PrimeKeyTablePkCol , string.Empty  );

            ExceptionWasThrown = false;
            try
            {
                CswTableUpdate CswTableUpdate = _CswNbtSchemaModTrnsctn.makeCswTableUpdate( "CswScmUpdt_TstCse_Constraint_RollbackDrop_update2", _PrimeKeyTable );
                DataTable PkTableTable = CswTableUpdate.getTable();
            ...
            if ( ExceptionWasThrown )
                throw ( new CswScmUpdt_Exception( "An exception was thrown due to a foreign key constraint violation on table table " + _ForeignKeyTable + "; this constraint should have been removed" ) );
            ...
            try
            {
                CswTableUpdate CswTableUpdate = _CswNbtSchemaModTrnsctn.makeCswTableUpdate( "CswScmUpdt_TstCse_Constraint_RollbackDrop_update3",_PrimeKeyTable );
                DataTable PkTableTable = CswTableUpdate.getTable();
            ...
            if ( !ExceptionWasThrown )
                throw ( new CswScmUpdt_Exception( "No exception was thrown after the removal of a foreign key constraint was rolled back on table table " + _ForeignKeyTable ) );
