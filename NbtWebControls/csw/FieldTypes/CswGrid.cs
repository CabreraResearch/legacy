        ...
        private void CswGrid_DataBinding(object sender, EventArgs e)
        {
            try
            {
                if( Prop != null )
                {
                    ...
                    if( _View != null )
                    {
                        _Grid.ParentNodeKey = new CswNbtNodeKey( _CswNbtResources, null, Prop.NodeId, NodeSpecies.Plain, Prop.NodeTypeProp.NodeTypeId, Prop.NodeTypeProp.NodeType.ObjectClassId, string.Empty, string.Empty );
                        _Grid.View = _View;
