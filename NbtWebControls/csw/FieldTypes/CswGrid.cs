        ...
        private void CswGrid_DataBinding(object sender, EventArgs e)
        {
            try
            {
                if( Prop != null )
                {
                    ...
                    if( _View != null )
                    {
                        _Grid.ParentNodeKey = new CswNbtNodeKey( _CswNbtResources, string.Empty, Prop.NodeId, NodeSpecies.Plain, Prop.NodeTypeProp.NodeType.NodeTypeId, Prop.NodeTypeProp.NodeType.ObjectClass.ObjectClassId, string.Empty, string.Empty );
                        _Grid.View = _View;
