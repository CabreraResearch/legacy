...
<xsl:template match="NbtTree">
  <xsl:element name="Nodes">
    <xsl:apply-templates>
      ...
      <xsl:with-param name="issecondlevel">false</xsl:with-param>
    </xsl:apply-templates>
  </xsl:element>
</xsl:template>
...
<xsl:template match="NbtNode">
  ...
  <xsl:choose>
    <xsl:when test="$issecondlevel='true'">
    ...
    <xsl:otherwise>
      <xsl:apply-templates>
        ...
        <xsl:with-param name="issecondlevel" select="$isfirstlevel" />
      </xsl:apply-templates>
    </xsl:otherwise>
  </xsl:choose>
