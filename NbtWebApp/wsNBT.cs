...
namespace ChemSW.Nbt.WebServices
{
    ...
    public class wsNBT: WebService
    {
        ...
        [WebMethod( EnableSession = false )]
        [ScriptMethod( ResponseFormat = ResponseFormat.Json )]
        public string saveProps( string EditMode, string NodeId, string SafeNodeKey, string TabId, string NewPropsJson, string IdentityTabJson, string NodeTypeId, string ViewId, bool setIsTempToFalse )
        {
            JObject ReturnVal = new JObject();
            ...
            try
            {
                ...
                if( AuthenticationStatus.Authenticated == AuthenticationStatus )
                {
                    ...
                    ReturnVal = ws.saveProps( NodePk, CswConvert.ToInt32( TabId ), NewPropsObj, CswConvert.ToInt32( NodeTypeId ), View, setIsTempToFalse, IsIdentityTab : false  );
                }
                _deInitResources();
            }
            catch( Exception Ex )
