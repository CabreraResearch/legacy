...
(function _dataTypeIIFE(n$) {
    ...
    */
    var _DataType = function (name, type, defaultValue) {
        var that = this;
    ...
    n$.instanceOf.register('DataType', _DataType);
     ...
     * @param defaultValue {String} [defaultValue] A default value
    */
    n$.dataTypes.register('_dataType', function (name, type, defaultValue) {
        if (!name) {
            throw new Error('Cannot create a dataType without a name');
        }
        var ret = new _DataType(name, type, defaultValue);
        return ret;
    });
