...
function(m,n){a[l].data[n]={}});b.each(a[l]._get_settings().plugins,function(m,n){e[n]&&e[n].__init.apply(a[l])});a[l].init();b(this).trigger("init_done.jstree")});return j};b.jstree={defaults:{plugins:[]},_focused:function(){return a[d]||null},_reference:function(c){if(a[c])return a[c];var g=b(c);if(!g.length&&typeof c==="string")g=b("#"+c);if(!g.length)return null;return a[g.closest(".jstree").data("jstree-instance-id")]||null},_instance:function(c,g,h){this.data={core:{}};this.get_settings=function(){return b.extend(true,
...
[];this.get_container().find(".jstree-open").each(function(){c.data.core.to_open.push("#"+this.id.toString().replace(/^#/,"").replace("\\/","/").replace("/","\\/"))});this.__callback(c.data.core.to_open)},reopen:function(c){var g=this,h=[],j=[];if(!c){this.data.core.reopen=false;this.data.core.refreshing=true}if(this.data.core.to_open.length){b.each(this.data.core.to_open,function(l,k){if(k=="#")return true;b(k).length&&b(k).is(".jstree-closed")?h.push(k):j.push(k)});if(h.length){this.data.core.to_open=
j;b.each(h,function(l,k){g.open_node(k,function(){g.reopen(true)},true)})}}},refresh:function(c){var g=this;this.save_opened();c||(c=-1);(c=this._get_node(c))||(c=-1);c!==-1&&c.children("UL").remove();this.load_node(c,function(){g.__callback({obj:c});g.reopen()})},loaded:function(){this.__callback()},set_focus:function(){var c=b.jstree._focused();c&&c!==this&&c.get_container().removeClass("jstree-focused");if(c!==this){this.get_container().addClass("jstree-focused");d=this.get_index()}this.__callback()},
is_focused:function(){return d==this.get_index()},_get_node:function(c){var g=b(c,this.get_container());if(g.is(".jstree")||c==-1)return-1;g=g.closest("li",this.get_container());return g.length?g:false},_get_next:function(c,g){c=this._get_node(c);if(c===-1)return this.get_container().find("> ul > li:first-child");if(!c.length)return false;if(g)return c.nextAll("li").size()>0?c.nextAll("li:eq(0)"):false;return c.hasClass("jstree-open")?c.find("li:eq(0)"):c.nextAll("li").size()>0?c.nextAll("li:eq(0)"):
c.parentsUntil(".jstree","li").next("li").eq(0)},_get_prev:function(c,g){c=this._get_node(c);if(c===-1)return this.get_container().find("> ul > li:last-child");if(!c.length)return false;if(g)return c.prevAll("li").length>0?c.prevAll("li:eq(0)"):false;if(c.prev("li").length){for(c=c.prev("li").eq(0);c.hasClass("jstree-open");)c=c.children("ul:eq(0)").children("li:last");return c}else{var h=c.parentsUntil(".jstree","li:eq(0)");return h.length?h:false}},_get_parent:function(c){c=this._get_node(c);if(c==
-1||!c.length)return false;c=c.parentsUntil(".jstree","li:eq(0)");return c.length?c:-1},_get_children:function(c){c=this._get_node(c);if(c===-1)return this.get_container().children("ul:eq(0)").children("li");if(!c.length)return false;return c.children("ul:eq(0)").children("li")},get_path:function(c,g){var h=[],j=this;c=this._get_node(c);if(c===-1||!c||!c.length)return false;c.parentsUntil(".jstree","li").each(function(){h.push(g?this.id:j.get_text(this))});h.reverse();h.push(g?c.attr("id"):this.get_text(c));
return h},is_open:function(c){return(c=this._get_node(c))&&c!==-1&&c.hasClass("jstree-open")},is_closed:function(c){return(c=this._get_node(c))&&c!==-1&&c.hasClass("jstree-closed")},is_leaf:function(c){return(c=this._get_node(c))&&c!==-1&&c.hasClass("jstree-leaf")},open_node:function(c,g,h){c=this._get_node(c);if(!c.length)return false;if(!c.hasClass("jstree-closed")){g&&g.call();return false}var j=h||i?0:this._get_settings().core.animation,l=this;if(this._is_loaded(c)){j&&c.children("ul").css("display",
"none");c.removeClass("jstree-closed").addClass("jstree-open").children("a").removeClass("jstree-loading");j&&c.children("ul").stop(true).slideDown(j,function(){this.style.display=""});this.__callback({obj:c});g&&g.call()}else{c.children("a").addClass("jstree-loading");this.load_node(c,function(){l.open_node(c,g,h)},g)}},close_node:function(c,g){c=this._get_node(c);var h=g||i?0:this._get_settings().core.animation;if(!c.length||!c.hasClass("jstree-open"))return false;h&&c.children("ul").attr("style",
"display:block !important");c.removeClass("jstree-open").addClass("jstree-closed");h&&c.children("ul").stop(true).slideUp(h,function(){this.style.display=""});this.__callback({obj:c})},toggle_node:function(c){c=this._get_node(c);if(c.hasClass("jstree-closed"))return this.open_node(c);if(c.hasClass("jstree-open"))return this.close_node(c)},open_all:function(c,g){c=c?this._get_node(c):this.get_container();if(!c||c===-1)c=this.get_container();if(g)c=c.find("li.jstree-closed");else{g=c;c=c.is(".jstree-closed")?
c.find("li.jstree-closed").andSelf():c.find("li.jstree-closed")}var h=this;c.each(function(){var j=this;h._is_loaded(this)?h.open_node(this,false,true):h.open_node(this,function(){h.open_all(j,g)},true)});g.find("li.jstree-closed").length===0&&this.__callback({obj:g})},close_all:function(c){var g=this;c=c?this._get_node(c):this.get_container();if(!c||c===-1)c=this.get_container();c.find("li.jstree-open").andSelf().each(function(){g.close_node(this)});this.__callback({obj:c})},clean_node:function(c){c=
c&&c!=-1?b(c):this.get_container();c=c.is("li")?c.find("li").andSelf():c.find("li");c.removeClass("jstree-last").filter("li:last-child").addClass("jstree-last").end().filter(":has(li)").not(".jstree-open").removeClass("jstree-leaf").addClass("jstree-closed");c.not(".jstree-open, .jstree-closed").addClass("jstree-leaf").children("ul").remove();this.__callback({obj:c})},get_rollback:function(){this.__callback();return{i:this.get_index(),h:this.get_container().children("ul").clone(true),d:this.data}},
set_rollback:function(c,g){this.get_container().empty().append(c);this.data=g;this.__callback()},load_node:function(c){this.__callback({obj:c})},_is_loaded:function(){return true},create_node:function(c,g,h,j,l){c=this._get_node(c);g=typeof g==="undefined"?"last":g;var k=b("<li>"),m=this._get_settings().core,n;if(c!==-1&&!c.length)return false;if(!l&&!this._is_loaded(c)){this.load_node(c,function(){this.create_node(c,g,h,j,true)});return false}this.__rollback();if(typeof h==="string")h={data:h};h||
(h={});h.attr&&k.attr(h.attr);h.state&&k.addClass("jstree-"+h.state);if(!h.data)h.data=m.strings.new_node;if(!b.isArray(h.data)){n=h.data;h.data=[];h.data.push(n)}b.each(h.data,function(p,o){n=b("<a>");if(b.isFunction(o))o=o.call(this,h);if(typeof o=="string")n.attr("href","#")[m.html_titles?"html":"text"](o);else{if(!o.attr)o.attr={};if(!o.attr.href)o.attr.href="#";n.attr(o.attr)[m.html_titles?"html":"text"](o.title);o.language&&n.addClass(o.language)}n.prepend("<ins class='jstree-icon'>&#160;</ins>");
if(o.icon)o.icon.indexOf("/")===-1?n.children("ins").addClass(o.icon):n.children("ins").css("background","url('"+o.icon+"') center center no-repeat");k.append(n)});k.prepend("<ins class='jstree-icon'>&#160;</ins>");if(c===-1){c=this.get_container();if(g==="before")g="first";if(g==="after")g="last"}switch(g){case "before":c.before(k);n=this._get_parent(c);break;case "after":c.after(k);n=this._get_parent(c);break;case "inside":case "first":c.children("ul").length||c.append("<ul>");c.children("ul").prepend(k);
n=c;break;case "last":c.children("ul").length||c.append("<ul>");c.children("ul").append(k);n=c;break;default:c.children("ul").length||c.append("<ul>");g||(g=0);n=c.children("ul").children("li").eq(g);n.length?n.before(k):c.children("ul").append(k);n=c}if(n===-1||n.get(0)===this.get_container().get(0))n=-1;this.clean_node(n);this.__callback({obj:k,parent:n});j&&j.call(this,k);return k},get_text:function(c){c=this._get_node(c);if(!c.length)return false;var g=this._get_settings().core.html_titles;c=
c.children("a:eq(0)");if(g){c=c.clone();c.children("INS").remove();return c.html()}else{c=c.contents().filter(function(){return this.nodeType==3})[0];return c.nodeValue}},set_text:function(c,g){c=this._get_node(c);if(!c.length)return false;c=c.children("a:eq(0)");if(this._get_settings().core.html_titles){var h=c.children("INS").clone();c.html(g).prepend(h);this.__callback({obj:c,name:g});return true}else{c=c.contents().filter(function(){return this.nodeType==3})[0];this.__callback({obj:c,name:g});
return c.nodeValue=g}},rename_node:function(c,g){c=this._get_node(c);this.__rollback();c&&c.length&&this.set_text.apply(this,Array.prototype.slice.call(arguments))&&this.__callback({obj:c,name:g})},delete_node:function(c){c=this._get_node(c);if(!c.length)return false;this.__rollback();var g=this._get_parent(c),h=this._get_prev(c);c=c.remove();g!==-1&&g.find("> ul > li").length===0&&g.removeClass("jstree-open jstree-closed").addClass("jstree-leaf");this.clean_node(g);this.__callback({obj:c,prev:h});
return c},prepare_move:function(c,g,h,j,l){var k={};k.ot=b.jstree._reference(k.o)||this;k.o=k.ot._get_node(c);k.r=g===-1?-1:this._get_node(g);k.p=typeof k==="undefined"?"last":h;if(!(!l&&f.o&&f.o[0]===k.o[0]&&f.r[0]===k.r[0]&&f.p===k.p)){k.ot=b.jstree._reference(k.o)||this;k.rt=g===-1?k.ot:b.jstree._reference(k.r)||this;if(k.r===-1){k.cr=-1;switch(k.p){case "first":case "before":case "inside":k.cp=0;break;case "after":case "last":k.cp=k.rt.get_container().find(" > ul > li").length;break;default:k.cp=
k.p}}else{if(!/^(before|after)$/.test(k.p)&&!this._is_loaded(k.r))return this.load_node(k.r,function(){this.prepare_move(c,g,h,j,true)});switch(k.p){case "before":k.cp=k.r.index();k.cr=k.rt._get_parent(k.r);break;case "after":k.cp=k.r.index()+1;k.cr=k.rt._get_parent(k.r);break;case "inside":case "first":k.cp=0;k.cr=k.r;break;case "last":k.cp=k.r.find(" > ul > li").length;k.cr=k.r;break;default:k.cp=k.p;k.cr=k.r}}k.np=k.cr==-1?k.rt.get_container():k.cr;k.op=k.ot._get_parent(k.o);k.or=k.np.find(" > ul > li:nth-child("+
(k.cp+1)+")");f=k}this.__callback(f);j&&j.call(this,f)},check_move:function(){var c=f,g=true;if(c.or[0]===c.o[0])return false;c.o.each(function(){if(c.r.parentsUntil(".jstree").andSelf().filter("li").index(this)!==-1)return g=false});return g},move_node:function(c,g,h,j,l,k){if(!l)return this.prepare_move(c,g,h,function(n){this.move_node(n,false,false,j,true,k)});if(!k&&!this.check_move())return false;this.__rollback();g=false;if(j){g=c.o.clone();g.find("*[id]").andSelf().each(function(){if(this.id)this.id=
...
"",fontStyle:j.css("fontStyle")||"",fontStretch:j.css("fontStretch")||"",fontVariant:j.css("fontVariant")||"",letterSpacing:j.css("letterSpacing")||"",wordSpacing:j.css("wordSpacing")||""});j.width(Math.min(h.text("pW"+j[0].value).width(),f))[0].select()},rename:function(a){a=this._get_node(a);this.__rollback();var d=this.__callback;this._show_input(a,function(e,f,i){d.call(this,{obj:e,new_name:f,old_name:i})})},create:function(a,d,e,f,i,c){var g=this;(a=this._get_node(a))||(a=-1);this.__rollback();
return this.create_node(a,d,e,function(h){var j=this._get_parent(h),l=b(h).index();f&&f.call(this,h);j.length&&j.hasClass("jstree-closed")&&this.open_node(j,false,true);i?g.__callback({obj:h,name:this.get_text(h),parent:j,position:l}):this._show_input(h,function(k,m){g.__callback({obj:k,name:m,parent:j,position:l})})},c)},remove:function(a){a=this._get_node(a,true);this.__rollback();this.delete_node(a);this.__callback({obj:a})},check_move:function(){if(!this.__call_old())return false;if(!this._get_settings().crrm.move.check_move.call(this,
...
a.removeClass("jstree-open jstree-closed").addClass("jstree-leaf")}else h==="success"&&f.correct_state&&this.get_container().children("ul").empty();e&&e.call(this)};f.ajax.context=this;f.ajax.error=c;f.ajax.success=function(g,h,j){var l=this.get_settings().json_data.ajax.success;if(l)g=l.call(this,g,h,j)||g;if(g===""||!b.isArray(g)&&!b.isPlainObject(g))return c.call(this,j,h,"");if(g=this._parse_json(g)){if(a===-1||!a)this.get_container().children("ul").empty().append(g.children());else{a.append(g).children(".jstree-loading").removeClass("jstree-loading");
a.data("jstree-is-loading",false)}this.clean_node(a);d&&d.call(this)}else if(a===-1||!a){if(f.correct_state){this.get_container().children("ul").empty();d&&d.call(this)}}else{a.children(".jstree-loading").removeClass("jstree-loading");a.data("jstree-is-loading",false);if(f.correct_state){a.removeClass("jstree-open jstree-closed").addClass("jstree-leaf");d&&d.call(this)}}};if(!f.ajax.dataType)f.ajax.dataType="json";if(b.isFunction(f.ajax.url))f.ajax.url=f.ajax.url.call(this,a);if(b.isFunction(f.ajax.data))f.ajax.data=
f.ajax.data.call(this,a);b.ajax(f.ajax)}},_parse_json:function(a,d){var e=false,f=this._get_settings(),i=f.json_data,c=f.core.html_titles,g;if(!a)return e;if(b.isFunction(a))a=a.call(this);if(b.isArray(a)){e=b();if(!a.length)return false;f=0;for(i=a.length;f<i;f++){g=this._parse_json(a[f],true);if(g.length)e=e.add(g)}}else{if(typeof a=="string")a={data:a};if(!a.data&&a.data!=="")return e;e=b("<li>");a.attr&&e.attr(a.attr);a.metadata&&e.data("jstree",a.metadata);a.state&&e.addClass("jstree-"+a.state);
if(!b.isArray(a.data)){g=a.data;a.data=[];a.data.push(g)}b.each(a.data,function(h,j){g=b("<a>");if(b.isFunction(j))j=j.call(this,a);if(typeof j=="string")g.attr("href","#")[c?"html":"text"](j);else{if(!j.attr)j.attr={};if(!j.attr.href)j.attr.href="#";g.attr(j.attr)[c?"html":"text"](j.title);j.language&&g.addClass(j.language)}g.prepend("<ins class='jstree-icon'>&#160;</ins>");if(!j.icon&&a.icon)j.icon=a.icon;if(j.icon)j.icon.indexOf("/")===-1?g.children("ins").addClass(j.icon):g.children("ins").css("background",
"url('"+j.icon+"') center center no-repeat");e.append(g)});e.prepend("<ins class='jstree-icon'>&#160;</ins>");if(a.children)if(i.progressive_render&&a.state!=="open")e.addClass("jstree-closed").data("jstree-children",a.children);else{if(b.isFunction(a.children))a.children=a.children.call(this,a);if(b.isArray(a.children)&&a.children.length){g=this._parse_json(a.children,true);if(g.length){f=b("<ul>");f.append(g);e.append(f)}}}}if(!d){f=b("<ul>");f.append(e);e=f}return e},get_json:function(a,d,e,f){var i=
[],c=this._get_settings(),g=this,h,j,l,k,m,n;a=this._get_node(a);if(!a||a===-1)a=this.get_container().find("> ul > li");d=b.isArray(d)?d:["id","class"];!f&&this.data.types&&d.push(c.types.type_attr);e=b.isArray(e)?e:[];a.each(function(){l=b(this);h={data:[]};if(d.length)h.attr={};b.each(d,function(p,o){if((j=l.attr(o))&&j.length&&j.replace(/jstree[^ ]*|$/ig,"").length)h.attr[o]=j.replace(/jstree[^ ]*|$/ig,"")});if(l.hasClass("jstree-open"))h.state="open";if(l.hasClass("jstree-closed"))h.state="closed";
k=l.children("a");k.each(function(){m=b(this);if(e.length||b.inArray("languages",c.plugins)!==-1||m.children("ins").get(0).style.backgroundImage.length||m.children("ins").get(0).className&&m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").length){n=false;b.inArray("languages",c.plugins)!==-1&&b.isArray(c.languages)&&c.languages.length&&b.each(c.languages,function(p,o){if(m.hasClass(o)){n=o;return false}});j={attr:{},title:g.get_text(m,n)};b.each(e,function(p,o){h.attr[o]=(m.attr(o)||
"").replace(/jstree[^ ]*|$/ig,"")});b.each(c.languages,function(p,o){if(m.hasClass(o)){j.language=o;return true}});if(m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").replace(/^\s+$/ig,"").length)j.icon=m.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,"").replace(/^\s+$/ig,"");if(m.children("ins").get(0).style.backgroundImage.length)j.icon=m.children("ins").get(0).style.backgroundImage.replace("url(","").replace(")","")}else j=g.get_text(m);if(k.length>1)h.data.push(j);
else h.data=j});l=l.find("> ul > li");if(l.length)h.children=g.get_json(l,d,e,true);i.push(h)});return i}}})})(jQuery);
...
i.ajax.context=this;i.ajax.error=c;i.ajax.success=function(g,h,j){var l=this.get_settings().html_data.ajax.success;if(l)g=l.call(this,g,h,j);if(g=="")return c.call(this,j,h,"");if(g){g=b(g);g.is("ul")||(g=b("<ul>").append(g));if(a==-1||!a)this.get_container().children("ul").empty().append(g.children()).find("li, a").filter(function(){return this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");
else{a.children(".jstree-loading").removeClass("jstree-loading");a.append(g).find("li, a").filter(function(){return this.firstChild.tagName!=="INS"}).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon");a.data("jstree-is-loading",false)}this.clean_node(a);d&&d.call(this)}else if(a&&a!==-1){a.children(".jstree-loading").removeClass("jstree-loading");a.data("jstree-is-loading",false);if(i.correct_state){a.removeClass("jstree-open jstree-closed").addClass("jstree-leaf");
d&&d.call(this)}}else if(i.correct_state){this.get_container().children("ul").empty();d&&d.call(this)}};if(!i.ajax.dataType)i.ajax.dataType="html";if(b.isFunction(i.ajax.url))i.ajax.url=i.ajax.url.call(this,a);if(b.isFunction(i.ajax.data))i.ajax.data=i.ajax.data.call(this,a);b.ajax(i.ajax)}}}});b.jstree.defaults.plugins.push("html_data")})(jQuery);
