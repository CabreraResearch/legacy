...
g+" does not exist on CswWizard")}})(jQuery);function CswAjaxJSON(a){var d={url:"",data:{},onloginfail:function(){},success:function(){},error:function(){},formobile:false};a&&$.extend(d,a);$.ajax({type:"POST",url:d.url,dataType:"json",contentType:"application/json; charset=utf-8",data:JSON.stringify(d.data),success:function(b,e,g){b=$.parseJSON(b.d);if(b.error!==undefined){_handleAjaxError(g,{message:b.error.message,detail:b.error.detail},"");d.error()}else if(d.formobile){g=tryParseString(b.AuthenticationStatus,"");_handleAuthenticationStatus({status:g,
success:d.success(b),failure:d.onloginfail})}else{tryParseString(b.AuthenticationStatus,"");d.success(b)}},error:function(b,e,g){_handleAjaxError(b,{message:"A Webservices Error Occurred",detail:e},g);d.error()}})}
function CswAjaxXml(a){var d={url:"",data:{},stringify:false,onloginfail:function(){},success:function(){},error:function(){},formobile:false};a&&$.extend(d,a);if(!isNullOrEmpty(d.url)){a=$.param(d.data);$.ajax({type:"POST",url:d.url,dataType:"xml",data:a,success:function(b,e,g){e=$(g.responseXML);b=e.children().first();b.CswAttrXml("authenticationstatus");if(b.first().get(0).nodeName==="error"){_handleAjaxError(g,{message:b.CswAttrXml("message"),detail:b.CswAttrXml("detail")},"");d.error()}else if(d.formobile){g=
tryParseString(e.find("AuthenticationStatus").text(),"");_handleAuthenticationStatus({status:g,success:d.success(b),failure:d.onloginfail})}else d.success(b)},error:function(b,e,g){_handleAjaxError(b,{message:"A Webservices Error Occurred",detail:e},g);d.error()}})}}function _handleAjaxError(a,d){var b=$("#ErrorDiv");b.length>0?b.CswErrorMessage({message:d.message,detail:d.detail}):log(d.message+"; "+d.detail)}
