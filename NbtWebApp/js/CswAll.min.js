...
j.append(q.label);for(j=0;j<g.cols.length;j++){k=o.CswTable("cell",i+e,j+2);k.addClass("cbarraycell");m=g.ID+"_"+e+"_"+j;k=a('<input type="'+f+'" class="CBACheckBox_'+g.ID+'" id="'+m+'" name="'+g.ID+'" />').appendTo(k).click(g.onchange);k.CswAttrDom("key",q.key);k.CswAttrDom("rowlabel",q.label);k.CswAttrDom("collabel",g.cols[j]);k.CswAttrDom("row",e);k.CswAttrDom("col",j);q.values[j]&&k.CswAttrDom("checked","true")}}if(!g.UseRadios){e="Check All";if(a(".CBACheckBox_"+g.ID).not(":checked").length===
0)e="Uncheck All";var l=a('<div style="text-align: right"><a href="#">'+e+"</a></div>").appendTo(h).children("a");l.click(function(){var s=g.ID;if(a(".CBACheckBox_"+s).not(":checked").length>0){a(".CBACheckBox_"+s).CswAttrDom("checked","checked");l.text("Uncheck all")}else{a(".CBACheckBox_"+s).removeAttr("checked");l.text("Check all")}return false})}}},getdata:function(e){var g={ID:""};e&&a.extend(g,e);var f=[];a(this).find(".CBACheckBox_"+g.ID).each(function(){var h=a(this),i=parseInt(h.CswAttrDom("row")),
...
"object"||!b)return d.init.apply(this,arguments);else a.error("Method "+b+" does not exist on CswInput")}})(jQuery);(function(a){a.fn.CswLayoutTable=function(d){function b(k,m,l){for(var s=parseInt(k.CswAttrDom("cellset_rows")),x=parseInt(k.CswAttrDom("cellset_columns")),A=[],v=1;v<=s;v++)for(var w=1;w<=x;w++){if(A[v]===undefined)A[v]=[];A[v][w]=i(k,m,l,v,w,s,x)}return A}function e(k){return k.CswAttrDom("removemode")==="true"}function g(k){return k.CswAttrDom("configmode")==="true"}function f(k,m){m.find("#"+k.CswAttrDom("id")+"addbtn").hide();m.find("#"+k.CswAttrDom("id")+"rembtn").hide();m.find("#"+k.CswAttrDom("id")+
...
parseInt(k.CswAttrDom("cellset_columns"));k.CswTable("finish",null);k.CswTable("findCell",".CswLayoutTable_cell").addClass("CswLayoutTable_configcell");k.CswAttrDom("configmode","true");k.trigger(k.CswAttrDom("id")+"CswLayoutTable_onConfigOn",m)}function i(k,m,l,s,x,A,v){if(m<1)m=1;if(l<1)l=1;return k.CswTable("cell",(m-1)*A+s,(l-1)*v+x)}function q(k,m,l,s,x,A){var v=Math.ceil(l/x),w=Math.ceil(s/A);l=parseInt(x-l%x);s=parseInt(A-s%A);m.CswAttrDom("row",v).CswAttrDom("column",w).CswAttrDom("cellsetrow",
...
function(){var t=a(this);g(k)&&t.removeClass("CswLayoutTable_hover");e(k)&&k.CswTable("findCell",'[row="'+t.CswAttrDom("row")+'"][column="'+t.CswAttrDom("column")+'"]').removeClass("CswLayoutTable_remove")})}function j(k){var m=k.find(".CswLayoutTable_hover");return k.CswTable("findCell",'[row="'+m.CswAttrDom("row")+'"][column="'+m.CswAttrDom("column")+'"]')}var o={init:function(k){var m={ID:"",cellset:{rows:1,columns:1},onSwap:function(){},onAddClick:function(){},onConfigOn:function(){},onConfigOff:function(){},
...
width:m.width,align:m.align,onCreateCell:function(A,v,w,t,z){q(v,w,t,z,m.cellset.rows,m.cellset.columns)}}).CswAttrDom("cellset_rows",m.cellset.rows).CswAttrDom("cellset_columns",m.cellset.columns);s.CswAttrDom("configmode","false");s.bind(m.ID+"CswLayoutTable_onSwap",m.onSwap);s.bind(m.ID+"CswLayoutTable_onRemove",m.onRemove);s.bind(m.ID+"CswLayoutTable_onConfigOn",m.onConfigOn);s.bind(m.ID+"CswLayoutTable_onConfigOff",m.onConfigOff);var x=l.CswTable("init",{ID:m.ID+"_buttontbl"});m.showAddButton&&
x.CswTable("cell",1,1).CswImageButton({ButtonType:CswImageButton_ButtonType.Add,AlternateText:"Add",ID:m.ID+"addbtn",onClick:function(){m.onAddClick();return CswImageButton_ButtonType.None}}).hide();m.showRemoveButton&&x.CswTable("cell",1,2).CswImageButton({ButtonType:CswImageButton_ButtonType.Delete,AlternateText:"Remove",ID:m.ID+"rembtn",onClick:function(){var A=a(this);if(e(s)){s.CswAttrDom("removemode","false");A.removeClass("CswLayoutTable_removeEnabled")}else{s.CswAttrDom("removemode","true");
A.addClass("CswLayoutTable_removeEnabled")}return CswImageButton_ButtonType.None}}).hide();if(m.showConfigButton){x.CswTable("cell",1,3).CswImageButton({ButtonType:CswImageButton_ButtonType.ArrowEast,AlternateText:"Add Column",ID:m.ID+"addcolumnbtn",onClick:function(){var A=parseInt(s.CswAttrDom("cellset_rows")),v=parseInt(s.CswAttrDom("cellset_columns")),w=s.CswTable("maxrows"),t=s.CswTable("maxcolumns");i(s,w/A,t/v+1,A,v,A,v);s.CswTable("finish",null);g(s)&&s.CswTable("findCell",".CswLayoutTable_cell").addClass("CswLayoutTable_configcell");
return CswImageButton_ButtonType.None}}).hide();x.CswTable("cell",1,4).CswImageButton({ButtonType:CswImageButton_ButtonType.ArrowSouth,AlternateText:"Add Row",ID:m.ID+"addrowbtn",onClick:function(){var A=parseInt(s.CswAttrDom("cellset_rows")),v=parseInt(s.CswAttrDom("cellset_columns")),w=s.CswTable("maxrows"),t=s.CswTable("maxcolumns");i(s,w/A+1,t/v,A,v,A,v);s.CswTable("finish",null);g(s)&&s.CswTable("findCell",".CswLayoutTable_cell").addClass("CswLayoutTable_configcell");return CswImageButton_ButtonType.None}}).hide();
...
a(this);g.append('<option value="'+h.CswAttrXml("id")+'">'+h.CswAttrXml("name")+"</option>")});e.onSuccess()}});return g},value:function(){return a(this).val()}};a.fn.CswNodeSelect=function(b){if(d[b])return d[b].apply(this,Array.prototype.slice.call(arguments,1));else if(typeof b==="object"||!b)return d.init.apply(this,arguments);else a.error("Method "+b+" does not exist on CswNodeSelect")}})(jQuery);(function(a){a.fn.CswNodeTabs=function(d){function b(m,l){CswAjaxXml({url:o.PropsUrl,data:{EditMode:o.EditMode,NodeId:o.nodeid,TabId:l,SafeNodeKey:o.cswnbtnodekey,NodeTypeId:o.nodetypeid},stringify:false,success:function(s){var x=m.children("form");x.contents().remove();o.title!==""&&x.append(o.title);var A=x.CswLayoutTable("init",{ID:o.ID+"_props",OddCellRightAlign:true,ReadOnly:o.EditMode==="PrintReport",cellset:{rows:1,columns:2},onSwap:function(w,t){e(t)},showConfigButton:o.filterToPropId==="",
onConfigOn:function(){s.children().each(function(){var w=a(this),t=A.find("#"+w.CswAttrXml("id")+"_subproptable"),z=t.parent();z=A.CswLayoutTable("cellset",z.CswAttrDom("row"),z.CswAttrDom("column"))[1][2];if(t.length>0){t={fieldtype:w.CswAttrXml("fieldtype"),nodeid:o.nodeid,relatednodeid:o.relatednodeid,propid:w.CswAttrXml("id"),$propdiv:z.children("div"),$propxml:w,onchange:function(){},onReload:function(){b(m,l)},cswnbtnodekey:o.cswnbtnodekey};i(t,o.SinglePropUrl,o.EditMode,o.cswnbtnodekey,w.CswAttrXml("id"),
o.nodetypeid,w,z,m,l,true)}})},onConfigOff:function(){s.children().each(function(){var w=a(this),t=A.find("#"+w.CswAttrXml("id")+"_subproptable"),z=t.parent();z=A.CswLayoutTable("cellset",z.CswAttrDom("row"),z.CswAttrDom("column"))[1][2];if(t.length>0){t={fieldtype:w.CswAttrXml("fieldtype"),nodeid:o.nodeid,relatednodeid:o.relatednodeid,propid:w.CswAttrXml("id"),$propdiv:z.children("div"),$propxml:w,onchange:function(){},onReload:function(){b(m,l)},cswnbtnodekey:o.cswnbtnodekey};i(t,o.SinglePropUrl,
o.EditMode,o.cswnbtnodekey,w.CswAttrXml("id"),o.nodetypeid,w,z,m,l,false)}})}});f(A,s,m,l);if(o.EditMode!=="PrintReport")var v=x.CswButton({ID:"SaveTab",enabledText:"Save Changes",disabledText:"Saving...",onclick:function(){q(x,A,s,v)}});x.validate({highlight:function(w){w=a(w);w.CswAttrDom("csw_invalid","1");w.animate({backgroundColor:"#ff6666"})},unhighlight:function(w){var t=a(w);if(t.CswAttrDom("csw_invalid")==="1"){t.css("background-color","#66ff66");t.CswAttrDom("csw_invalid","0");setTimeout(function(){t.animate({backgroundColor:"transparent"})},
500)}}})}})}function e(m){g(m.cellset[1][2].children("div").first(),m.swaprow,m.swapcolumn);g(m.swapcellset[1][2].children("div").first(),m.row,m.column)}function g(m,l,s){if(m.length>0){m={PropId:m.CswAttrDom("propid"),NewRow:l,NewColumn:s,EditMode:o.EditMode};CswAjaxJSON({url:o.MovePropUrl,data:m,success:function(){}})}}function f(m,l,s,x,A){l.children().each(function(){var v=a(this),w=v.CswAttrXml("id"),t=v.CswAttrXml("fieldtype"),z=m.CswLayoutTable("cellset",v.CswAttrXml("displayrow"),v.CswAttrXml("displaycol"));
...
w)}w=z[1][2];w.addClass("propertyvaluecell");h(w,v,s,x,A)})}function h(m,l,s,x,A){m.empty();if((l.CswAttrXml("display")!=="false"||A)&&(o.filterToPropId===""||o.filterToPropId===l.CswAttrXml("id"))){var v={fieldtype:l.CswAttrXml("fieldtype"),nodeid:o.nodeid,relatednodeid:o.relatednodeid,propid:l.CswAttrXml("id"),$propdiv:a("<div/>").appendTo(m),$propxml:l,onchange:function(){},onReload:function(){b(s,x)},cswnbtnodekey:o.cswnbtnodekey,EditMode:o.EditMode,onEditView:o.onEditView};v.$propdiv.CswAttrDom("nodeid",
v.nodeid);v.$propdiv.CswAttrDom("propid",v.propid);v.$propdiv.CswAttrDom("cswnbtnodekey",v.cswnbtnodekey);v.onchange=function(){o.onPropertyChange(v.propid,l.CswAttrXml("name"))};if(l.CswAttrXml("hassubprops")==="true")v.onchange=function(){i(v,o.SinglePropUrl,o.EditMode,o.cswnbtnodekey,l.CswAttrXml("id"),o.nodetypeid,l,m,s,x,false);o.onPropertyChange(v.propid,l.CswAttrXml("name"))};a.CswFieldTypeFactory("make",v);var w=l.children("subprops"),t=m.CswLayoutTable("init",{ID:v.propid+"_subproptable",
OddCellRightAlign:true,ReadOnly:o.EditMode==="PrintReport",cellset:{rows:1,columns:2},onSwap:function(z,P){e(P)},showConfigButton:false});if(w.length>0&&w.children('[display != "false"]').length>0||A){f(t,w,s,x,A);A?t.CswLayoutTable("ConfigOn"):t.CswLayoutTable("ConfigOff")}}}function i(m,l,s,x,A,v,w,t,z,P,K){a.CswFieldTypeFactory("save",m);m={EditMode:s,NodeId:o.nodeid,SafeNodeKey:x,PropId:A,NodeTypeId:v,NewPropXml:xmlToString(w)};CswAjaxXml({url:l,data:m,stringify:true,success:function(fa){h(t,
fa.children().first(),z,P,K)}})}function q(m,l,s,x){if(m.valid()){j(l,s);m={EditMode:o.EditMode,NodeId:o.nodeid,SafeNodeKey:o.cswnbtnodekey,NodeTypeId:o.nodetypeid,ViewId:a.CswCookie("get",CswCookieName.CurrentViewId),NewPropsXml:xmlToString(s)};CswAjaxJSON({url:o.SavePropUrl,data:m,success:function(A){var v=true;if(o.ShowCheckboxes){var w=a("."+o.NodeCheckTreeId+"_check:checked"),t=a("."+o.ID+"_check:checked");if(w.length>0&&t.length>0){var z={SourceNodeKey:o.cswnbtnodekey,CopyNodeIds:[],PropIds:[]};
w.each(function(){var P=a(this).CswAttrDom("nodeid");z.CopyNodeIds.push(P)});t.each(function(){var P=a(this).CswAttrDom("propid");z.PropIds.push(P)});CswAjaxJSON({url:o.CopyPropValuesUrl,data:z})}else{v=false;alert("You have not selected any properties to save.")}}if(v)o.onSave(A.nodeid,A.cswnbtnodekey)},error:function(){x.CswButton("enable")}})}else x.CswButton("enable")}function j(m,l){l.children().each(function(){var s={$propxml:a(this),$propdiv:"",$propCell:"",fieldtype:"",nodeid:o.nodeid,cswnbtnodekey:o.cswnbtnodekey};
...
ShowCheckboxes:false,NodeCheckTreeId:"",onEditView:function(){}};d&&a.extend(o,d);d=a(this);var k=a('<div id="'+o.ID+'_tabdiv" />').appendTo(d);(function(){CswAjaxXml({url:o.TabsUrl,data:{EditMode:o.EditMode,NodeId:o.nodeid,SafeNodeKey:o.cswnbtnodekey,NodeTypeId:o.nodetypeid},stringify:false,success:function(m){k.contents().remove();var l=[],s=0,x=0;m.children().each(function(){$tab=a(this);if(o.EditMode==="PrintReport"||l.length===0)l[l.length]=a("<div><ul></ul></div>").appendTo(k);var t=l[l.length-
1];t.children("ul").append('<li><a href="#'+$tab.CswAttrXml("id")+'">'+$tab.CswAttrXml("name")+"</a></li>");t.append('<div id="'+$tab.CswAttrXml("id")+'"><form id="'+$tab.CswAttrXml("id")+'_form" /></div>');if($tab.CswAttrXml("id")===o.tabid)s=x;x++});for(var A in l){var v=l[A];v.tabs({selected:s,select:function(t,z){if(o.onBeforeTabSelect(K)){var P=a(v.children("div")[z.index]),K=P.CswAttrDom("id");b(P,K);o.onTabSelect(K)}else return false}});m=a(v.children("div")[v.tabs("option","selected")]);var w=
m.CswAttrDom("id");b(m,w);o.onTabSelect(w)}}})})(o);if(o.EditMode!=="PrintReport"){d=a('<div id="'+o.ID+'_linkdiv" align="right"/>').appendTo(d);a('<a href="#">As Report</a>').appendTo(d).click(function(){openPopup("NewNodeReport.html?nodeid="+o.nodeid+"&cswnbtnodekey="+o.cswnbtnodekey,600,800)})}return this}})(jQuery);(function(a){function d(f){var h={e:"",data:"",$treediv:"",IDPrefix:"",onSelectNode:function(){},onInitialSelectNode:function(){},viewid:"",UsePaging:"",forsearch:""};f&&a.extend(h,f);h.onSelectNode=h.onInitialSelectNode;b(h);f&&a.extend(h,f);h.$treediv.unbind("select_node.jstree");h.$treediv.bind("select_node.jstree",function(){return b(h)})}function b(f){var h={e:"",data:"",$treediv:"",IDPrefix:"",onSelectNode:function(){},viewid:"",UsePaging:"",forsearch:""};f&&a.extend(h,f);f=jsTreeGetSelected(h.$treediv);
...
l={dots:false}}else{k=o.find("item").first().find("item").first().CswAttrXml("id");l={dots:true}}else k=i+"root";s=o.find('item[id="'+k+'"]');var x=[],A=0;s.parents("item").andSelf().each(function(){x[A]=a(this).CswAttrXml("id");A++});s=o.find("types").text();s=a.parseJSON(s);o=o.find("tree").children("root");if(o.length>0){var v=function(t){var z=t.CswAttrXml("id").substring(i.length),P=t.children("content").children("name").text(),K='<li id="'+t.CswAttrXml("id")+'" ';K+='    rel="'+t.CswAttrXml("rel")+
...
"";a(P.responseXML).children().first().children().each(function(){K+=v(a(this))});return K}}},ui:{select_limit:1,initially_select:k},themes:l,core:{initially_open:x},types:{types:s,max_children:-2,max_depth:-2},plugins:m}).bind("load_node.jstree",function(){a("."+i+"check").unbind("click");a("."+i+"check").click(function(){return e(q,a(this))})}).bind("select_node.jstree",function(t,z){return d({e:t,data:z,$treediv:q,IDPrefix:i,onSelectNode:h.onSelectNode,onInitialSelectNode:h.onInitialSelectNode,
...
(function(a){a.fn.CswSearch=function(d){function b(m){var l={$link:"",advancedIsHidden:false};m&&a.extend(l,m);if("Advanced"===l.$link.text()||l.advancedIsHidden){a("."+ViewBuilder_CssClasses.subfield_select.name).each(function(){a(this).show()});a("."+ViewBuilder_CssClasses.filter_select.name).each(function(){a(this).show()});a("."+ViewBuilder_CssClasses.default_filter.name).each(function(){a(this).hide()});l.$link.text("Simple");l.advancedIsHidden=false}else if("Simple"===l.$link.text()||!l.advancedIsHidden){a("."+
ViewBuilder_CssClasses.subfield_select.name).each(function(){a(this).hide()});a("."+ViewBuilder_CssClasses.filter_select.name).each(function(){a(this).hide()});a("."+ViewBuilder_CssClasses.default_filter.name).each(function(){a(this).show()});l.$link.text("Advanced");l.advancedIsHidden=true}return l.advancedIsHidden}function e(){for(var m=2;m<=j.propsCount;){var l=j.$searchTable.CswTable("cell",m,1).CswAttrDom({align:"right"}),s=a("<span>&nbsp;and&nbsp;</span>");l.append(s);m++}var x=1;j.$propsXml.children("property").each(function(){var A=
...
IdPrefix:j.ID,NodeKey:j.cswnbtnodekey},stringify:false,success:function(m){j.$propsXml=m;g()}})}function h(){var m=j.$searchTable,l=j.bottomCell,s=l+1,x=j.bottomRow;if(j.searchtype==="nodetypesearch"){m=j.$searchTable.CswTable("cell",j.bottomRow,j.bottomCell).empty();s=makeId({prefix:j.ID,ID:"split_cell_table"});m=m.CswTable("init",{ID:s,cellpadding:1,cellspacing:1,cellalign:"left",align:"left"});l=1;s=2;x=1}l=m.CswTable("cell",x,l).empty();var A=makeId({ID:"clear_button",prefix:j.ID});l.CswButton({ID:A,
enabledText:"Clear",disabledText:"Clear",disableOnClick:false,onclick:function(){k.empty();var w=makeId({prefix:j.ID,ID:"search_tbl"});j.$searchTable=k.CswTable("init",{ID:w,cellpadding:1,cellspacing:1,cellalign:"center",align:"center"});switch(j.searchtype){case "nodetypesearch":g();case "viewsearch":e()}}});m=m.CswTable("cell",x,s).empty();s=makeId({ID:"advanced_options",prefix:j.ID});var v=m.CswLink("init",{ID:s,href:"#advanced",value:"Advanced"}).click(function(){j.advancedIsHidden=b({$link:v,
...
fieldtype:v,ID:j.ID,$parent:j.$searchTable,viewbuilderpropid:w,advancedIsHidden:j.advancedIsHidden});l.push(A)});m={viewbuilderprops:l};break;case "viewsearch":s=j.doViewSearchUrl;j.$propsXml.children("property").each(function(){var A=a(this);A.CswAttrXml("filtarbitraryid");A=A.CswViewPropFilter("getFilterJson",{ID:j.ID,$parent:j.$searchTable});l.push(A)});m={viewprops:l,viewid:j.viewid}}m&&CswAjaxJSON({url:s,data:{SearchJson:m},success:function(A){j.viewid=A.sessionviewid;j.searchtype="viewsearch";
...
h,i)},maxrows:function(){return b(a(this))},maxcolumns:function(){return e(a(this))},finish:function(h){for(var i=a(this),q=b(i),j=e(i),o=1;o<=q;o++)for(var k=1;k<=j;k++){var m=g(i,o,k);if(m.contents().length===0)h!==null?h(m,o,k):m.append("&nbsp;")}},findRow:function(h){var i=a(this).children("tbody").children("tr");if(h!==""&&h!==null)i=i.filter(h);return i},findCell:function(h){var i=a(this).children("tbody").children("tr").children("td");if(h!==""&&h!==null)i=i.filter(h);return i},rowFindCell:function(h,
...
C);O.CswAttrXml("width",ia.CswNumberTextBox("value"))}function g(C,F){var V=false;if(a("#"+l.ID+"_all:checked").length>0)V=true;w.text("");l.startingStep===1&&v.CswWizard("button","next","disable");CswAjaxJSON({url:l.ViewGridUrl,data:{All:V,SelectedViewId:F},success:function(N){t.empty();var R=a('<div id="'+l.ID+'_gp" style="width:100%; height:20px;" />').appendTo(t),D=a('<table id="'+l.ID+'_gt" />').appendTo(t);a.extend(N,{autowidth:true,height:180,onSelectRow:function(L,U){if(U){K.CswButton("enable");
fa.CswButton("enable")}else{K.CswButton("disable");fa.CswButton("disable")}w.text(i(D));v.CswWizard("button","next","enable")},pager:R});D.jqGrid(N).hideCol(l.ColumnFullViewId);if(!isNullOrEmpty(N.selectedpk)){D.setSelection(j(D,N.selectedpk));D.CswNodeGrid("scrollToSelectedRow")}C(D)}})}function f(C){var F="";return F=l.startingStep===1?q(C,l.ColumnFullViewId):l.viewid}function h(C){var F="";return F=l.startingStep===1?q(C,l.ColumnViewMode):l.viewmode}function i(C){var F="";return F=l.startingStep===
1?q(C,l.ColumnViewName):l.viewname}function q(C,F){var V=C.jqGrid("getGridParam","selrow");return C.jqGrid("getCell",V,F)}function j(C,F){var V=C.jqGrid("getCol",l.ColumnViewId,true),N=0,R;for(R in V)if(V[R].value.toString()===F.toString())N=V[R].id;return N}function o(C,F,V){var N,R,D;IsAdministrator({Yes:function(){C.CswTable("cell",F,1).append(V);var L=C.CswTable("cell",F,2),U=C.CswAttrDom("id");N=a('<select id="'+U+'_vissel" />').appendTo(L);N.append('<option value="User">User:</option>');N.append('<option value="Role">Role:</option>');
N.append('<option value="Global">Global</option>');R=L.CswNodeSelect("init",{ID:U+"_visrolesel",objectclass:"RoleClass"}).hide();D=L.CswNodeSelect("init",{ID:U+"_visusersel",objectclass:"UserClass"});N.change(function(){var I=N.val();if(I==="Role"){R.show();D.hide()}else if(I==="User"){R.hide();D.show()}else{R.hide();D.hide()}})}});return{getvisibilityselect:function(){return N},getvisroleselect:function(){return R},getvisuserselect:function(){return D}}}function k(C,F){var V=m(C,O);F.jstree({html_data:{data:V.htmlstring},
ui:{select_limit:1},types:{types:V.types,max_children:-2,max_depth:-2},plugins:["themes","html_data","ui","types","crrm"]});F.find(".vieweditor_childselect").change(function(){var D=a(this),L=D.find("option:selected").data("optionviewxml");D.CswAttrDom("arbid")==="root"?a(L).appendTo(O):a(L).appendTo(O.find('[arbitraryid="'+D.CswAttrDom("arbid")+'"]'));k(C,F)});F.find(".vieweditor_deletespan").each(function(){var D=a(this);D.CswImageButton({ButtonType:CswImageButton_ButtonType.Delete,AlternateText:"Delete",
ID:D.CswAttrDom("arbid")+"_delete",onClick:function(L){L=L.parent();O.find('[arbitraryid="'+L.CswAttrDom("arbid")+'"]').remove();k(C,F);return CswImageButton_ButtonType.None}})});C===CswViewEditor_WizardSteps.step5.step&&F.find(".vieweditor_addfilter").each(function(){var D=a(this),L=D.CswTable({ID:l.ID+"_propfilttbl"});L.css("display","inline-table");L.CswViewPropFilter("init",{viewxml:xmlToString(O),proparbitraryid:D.CswAttrDom("proparbid"),filtarbitraryid:"",viewbuilderpropid:"",ID:l.ID,propRow:1,
firstColumn:1,includePropertyName:false,selectedSubfieldVal:"",selectedFilterVal:"",autoFocusInput:false});L.CswTable("cell",1,5).CswButton("init",{ID:"addfiltbtn",prefix:l.ID,enabledText:"Add",disabledText:"Adding",onclick:function(){var U=L.CswViewPropFilter("getFilterJson",{ID:l.ID,$parent:D,proparbitraryid:D.CswAttrDom("proparbid")});L.CswViewPropFilter("makeFilter",{viewxml:xmlToString(O),filtJson:U,onSuccess:function(I){var T=O.find('[arbitraryid="'+D.CswAttrDom("proparbid")+'"]');a(xmlToString(I)).appendTo(T);
k(C,F)}})}})});if(C===CswViewEditor_WizardSteps.step6.step){var N=ma.CswTable("cell",1,2),R=h(z);F.find(".vieweditor_viewrootlink").click(function(){N.empty()});F.find(".vieweditor_viewrellink").click(function(){$a=a(this);N.empty();var D=O.find('[arbitraryid="'+$a.CswAttrDom("arbid")+'"]'),L=N.CswTable({ID:l.ID+"_editrel",FirstCellRightAlign:true});L.CswTable("cell",1,1).append("Allow Deleting");var U=L.CswTable("cell",1,2).CswInput("init",{ID:l.ID+"_adcb",type:CswInput_Types.checkbox});D.CswAttrXml("allowdelete").toLowerCase()==
"true"&&U.CswAttrDom("checked","true");L.CswTable("cell",2,1).append("Group By");var I=a('<select id="'+l.ID+'_gbs" />').appendTo(L.CswTable("cell",2,2)),T={Type:D.CswAttrXml("secondtype"),Id:D.CswAttrXml("secondid")};CswAjaxXml({url:l.PropNamesUrl,data:T,stringify:false,success:function(da){I.empty();a('<option value="">[None]</option>').appendTo(I);da.children().each(function(){var ca=a(this),aa=a('<option value="'+ca.CswAttrXml("propid")+'">'+ca.CswAttrXml("propname")+"</option>").appendTo(I).data("propxml",
ca);D.CswAttrXml("groupbypropid")===ca.CswAttrXml("propid")&&D.CswAttrXml("groupbyproptype")===ca.CswAttrXml("proptype")&&D.CswAttrXml("groupbypropname")===ca.CswAttrXml("propname")&&aa.CswAttrDom("selected","true")})}});var S;if(R==="Tree"){L.CswTable("cell",3,1).append("Show In Tree");S=L.CswTable("cell",3,2).CswInput("init",{ID:l.ID+"_stcb",type:CswInput_Types.checkbox});D.CswAttrXml("showintree").toLowerCase()=="true"&&S.CswAttrDom("checked","true")}L.CswTable("cell",4,2).CswButton({ID:l.ID+"_saverel",
enabledText:"Apply",disableOnClick:false,onclick:function(){S!==undefined&&D.CswAttrXml("showintree",S.is(":checked"));D.CswAttrXml("allowdelete",U.is(":checked"));if(I.val()!==""){var da=I.find(":selected").data("propxml");D.CswAttrXml("groupbypropid",da.CswAttrXml("propid"));D.CswAttrXml("groupbyproptype",da.CswAttrXml("proptype"));D.CswAttrXml("groupbypropname",da.CswAttrXml("propname"))}else{D.CswAttrXml("groupbypropid","");D.CswAttrXml("groupbyproptype","");D.CswAttrXml("groupbypropname","")}}})});
F.find(".vieweditor_viewproplink").click(function(){$a=a(this);N.empty();if(R==="Grid"){var D=O.find('[arbitraryid="'+$a.CswAttrDom("arbid")+'"]'),L=N.CswTable({ID:l.ID+"_editprop",FirstCellRightAlign:true});L.CswTable("cell",1,1).append("Sort By");var U=L.CswTable("cell",1,2).CswInput("init",{ID:l.ID+"_sortcb",type:CswInput_Types.checkbox});D.CswAttrXml("sortby").toLowerCase()=="true"&&U.CswAttrDom("checked","true");L.CswTable("cell",2,1).append("Grid Column Order");var I=L.CswTable("cell",2,2).CswInput("init",
{ID:l.ID+"_gcotb",type:CswInput_Types.text});I.val(D.CswAttrXml("order"));L.CswTable("cell",3,1).append("Grid Column Width (in characters)");var T=L.CswTable("cell",3,2).CswInput("init",{ID:l.ID+"_gcwtb",type:CswInput_Types.text});T.val(D.CswAttrXml("width"));L.CswTable("cell",4,2).CswButton({ID:l.ID+"_saveprop",enabledText:"Apply",disableOnClick:false,onclick:function(){D.CswAttrXml("sortby",U.is(":checked"));D.CswAttrXml("order",I.val());D.CswAttrXml("width",T.val())}})}});F.find(".vieweditor_viewfilterlink").click(function(){$a=
a(this);N.empty();var D=O.find('[arbitraryid="'+$a.CswAttrDom("arbid")+'"]'),L=N.CswTable({ID:l.ID+"_editfilt",FirstCellRightAlign:true});L.CswTable("cell",1,1).append("Case Sensitive");var U=a('<input type="checkbox" id="'+l.ID+'_casecb" />').appendTo(L.CswTable("cell",1,2));D.CswAttrXml("casesensitive").toLowerCase()==="true"&&U.CswAttrDom("checked","true");L.CswTable("cell",4,2).CswButton({ID:l.ID+"_savefilt",enabledText:"Apply",disableOnClick:false,onclick:function(){D.CswAttrXml("casesensitive",
U.is(":checked"))}})})}}function m(C,F){var V={},N=F.CswAttrXml("arbitraryid"),R=F.get(0).nodeName,D,L,U=false,I=true,T;if(R.toLowerCase()==="treeview"){if(C===CswViewEditor_WizardSteps.step3.step)I=false;N="root";D=F.CswAttrXml("viewname");L="root";V.root={icon:{image:F.CswAttrXml("iconfilename")}};T="vieweditor_viewrootlink"}else if(R.toLowerCase()==="relationship"){if(C===CswViewEditor_WizardSteps.step3.step)I=false;if(C===CswViewEditor_WizardSteps.step4.step)I=false;D=F.CswAttrXml("secondname");
R=F.CswAttrXml("propname");if(R!==""&&R!==undefined)D+=F.CswAttrXml("propowner")==="First"?" (by "+F.CswAttrXml("firstname")+"'s "+R+")":" (by "+R+")";L=F.CswAttrXml("secondtype")+"_"+F.CswAttrXml("secondid");V[L]={icon:{image:F.CswAttrXml("secondiconfilename")}};T="vieweditor_viewrellink"}else if(R.toLowerCase()==="property"){if(C<=CswViewEditor_WizardSteps.step3.step)U=true;if(C===CswViewEditor_WizardSteps.step5.step)I=false;D=F.CswAttrXml("name");L="property";V.property={icon:{image:"Images/view/property.gif"}};
T="vieweditor_viewproplink"}else if(R.toLowerCase()==="filter"){if(C<=CswViewEditor_WizardSteps.step4.step)U=true;D=F.CswAttrXml("subfieldname")+" "+F.CswAttrXml("filtermode")+" "+F.CswAttrXml("value");L="filter";V.filter={icon:{image:"Images/view/filter.gif"}};T="vieweditor_viewfilterlink"}var S="";if(!U){S='<li id="'+N+'" ';S+='    rel="'+L+'" ';S+='    class="jstree-open" ';S+=">";S+=' <a href="#" class="'+T+'" arbid="'+N+'">'+D+"</a>";if(N!=="root")S+=' <span style="" class="vieweditor_deletespan" arbid="'+
N+'"></span>';S+="<ul>";F.children().each(function(){var da=m(C,a(this));S+=da.htmlstring;a.extend(V,da.types)});if(!I)if(C===CswViewEditor_WizardSteps.step5.step)S+='<li><span class="vieweditor_addfilter" proparbid="'+N+'"></span></li>';else{S+='<li><select id="'+C+"_"+N+'_child" arbid="'+N+'" class="vieweditor_childselect"></select></li>';D={StepNo:C,ArbitraryId:N,ViewXml:xmlToString(O)};CswAjaxXml({url:l.ChildOptionsUrl,data:D,stringify:true,success:function(da){var ca=a("#"+C+"_"+N+"_child");
ca.empty();ca.append('<option value="">Select...</option>');da.children().each(function(){var aa=a(this),ba=a(aa.CswAttrXml("value"));a('<option value="'+ba.CswAttrXml("arbitraryid")+'">'+aa.CswAttrXml("name")+"</option>").appendTo(ca).data("optionviewxml",aa.CswAttrXml("value"))})}})}S+="</ul>";S+="</li>"}return{htmlstring:S,types:V}}var l={ViewGridUrl:"/NbtWebApp/wsNBT.asmx/getViewGrid",ViewInfoUrl:"/NbtWebApp/wsNBT.asmx/getViewInfo",SaveViewUrl:"/NbtWebApp/wsNBT.asmx/saveViewInfo",CopyViewUrl:"/NbtWebApp/wsNBT.asmx/copyView",
DeleteViewUrl:"/NbtWebApp/wsNBT.asmx/deleteView",ChildOptionsUrl:"/NbtWebApp/wsNBT.asmx/getViewChildOptions",PropNamesUrl:"/NbtWebApp/wsNBT.asmx/getPropNames",viewid:"",viewname:"",viewmode:"",ID:"vieweditor",ColumnViewName:"VIEWNAME",ColumnViewId:"NODEVIEWID",ColumnFullViewId:"VIEWID",ColumnViewMode:"VIEWMODE",onCancel:function(){},onFinish:function(){},startingStep:1};d&&a.extend(l,d);var s=[CswViewEditor_WizardSteps.step1,CswViewEditor_WizardSteps.step2,CswViewEditor_WizardSteps.step3,CswViewEditor_WizardSteps.step4,
CswViewEditor_WizardSteps.step5,CswViewEditor_WizardSteps.step6],x={};for(d=1;d<=s.length;d++)x[d]=s[d-1].description;var A=l.startingStep;d=a(this);d=a("<div></div>").appendTo(d);var v=d.CswWizard("init",{ID:l.ID+"_wizard",Title:"Edit View",StepCount:s.length,Steps:x,StartingStep:l.startingStep,FinishText:"Save and Finish",onNext:function(C){A=C;switch(C){case CswViewEditor_WizardSteps.step2.step:v.CswWizard("button","finish","enable");C={ViewId:f(z)};CswAjaxXml({url:l.ViewInfoUrl,data:C,stringify:false,
success:function(F){O=F;ja.val(O.CswAttrXml("viewname"));sa.val(O.CswAttrXml("category"));if(O.CswAttrXml("visibility")!=="Property")if(ea.getvisibilityselect()!==undefined){ea.getvisibilityselect().val(O.CswAttrXml("visibility")).trigger("change");ea.getvisroleselect().val("nodes_"+O.CswAttrXml("visibilityroleid"));ea.getvisuserselect().val("nodes_"+O.CswAttrXml("visibilityuserid"))}isTrue(O.CswAttrXml("formobile"))&&ra.CswAttrDom("checked","checked");F=O.CswAttrXml("mode");wa.text(F);ia.CswNumberTextBox("setValue",
O.CswAttrXml("width"));if(F==="Grid"){Z.show();ia.show()}else{Z.hide();ia.hide()}}});break;case CswViewEditor_WizardSteps.step3.step:O!==undefined&&e();k(CswViewEditor_WizardSteps.step3.step,ka);break;case CswViewEditor_WizardSteps.step4.step:k(CswViewEditor_WizardSteps.step4.step,ta);break;case CswViewEditor_WizardSteps.step5.step:k(CswViewEditor_WizardSteps.step5.step,la);break;case CswViewEditor_WizardSteps.step6.step:k(CswViewEditor_WizardSteps.step6.step,ma.CswTable("cell",1,1))}},onPrevious:function(C){C===
1&&v.CswWizard("button","finish","disable");A=C;switch(C){case CswViewEditor_WizardSteps.step3.step:k(CswViewEditor_WizardSteps.step3.step,ka);break;case CswViewEditor_WizardSteps.step4.step:k(CswViewEditor_WizardSteps.step4.step,ta);break;case CswViewEditor_WizardSteps.step5.step:k(CswViewEditor_WizardSteps.step5.step,la);break;case CswViewEditor_WizardSteps.step6.step:k(CswViewEditor_WizardSteps.step6.step,ma.CswTable("cell",1,1))}},onBeforePrevious:function(C){return C!==CswViewEditor_WizardSteps.step2.step||
confirm("You will lose any changes made to the current view if you continue.  Are you sure?")},onCancel:l.onCancel,onFinish:function(){var C=f(z);A===CswViewEditor_WizardSteps.step2.step&&!isNullOrEmpty(O)&&e();var F={ViewId:C,ViewXml:xmlToString(O)};CswAjaxXml({url:l.SaveViewUrl,data:F,stringify:true,success:function(){l.onFinish(C,h(z))}})}});l.startingStep===1&&v.CswWizard("button","finish","disable");s=v.CswWizard("div",CswViewEditor_WizardSteps.step1.step);s.append("A <em>View</em> controls the arrangement of information you see in a tree or grid.  Views are useful for defining a user's workflow or for creating elaborate search criteria. This wizard will take you step by step through the process of creating a new View or editing an existing View.<br/><br/>");
s.append("Select a View to Edit:&nbsp;");var w=a('<span id="'+l.ID+'_selviewname" style="font-weight: bold"></span>').appendTo(s),t=a("<div></div>").appendTo(s),z;g(b,l.viewid);x=s.CswTable({ID:l.ID+"_1_btntbl",width:"100%"});s=x.CswTable("cell",1,1);x=x.CswTable("cell",1,2);x.CswAttrDom("align","right");var P=a("<div></div>").appendTo(x);IsAdministrator({Yes:function(){P.CswInput("init",{ID:l.ID+"_all",type:CswInput_Types.checkbox,onChange:function(){g(b)}});P.append("Show Other Roles/Users")}});
var K=s.CswButton({ID:l.ID+"_copyview",enabledText:"Copy View",disableOnClick:true,onclick:function(){var C=f(z);isNullOrEmpty(C)||CswAjaxJSON({url:l.CopyViewUrl,data:{ViewId:C},success:function(F){g(b,F.copyviewid)},error:function(){K.CswButton("enable")}})}});K.CswButton("disable");var fa=s.CswButton({ID:l.ID+"_deleteview",enabledText:"Delete View",disableOnClick:true,onclick:function(){var C=f(z);isNullOrEmpty(C)||confirm("Are you sure you want to delete: "+i(z))&&CswAjaxJSON({url:l.DeleteViewUrl,
data:{ViewId:C},success:function(){g(b);K.CswButton("disable")},error:function(){fa.CswButton("enable")}})}});fa.CswButton("disable");var ga=s.CswButton({ID:l.ID+"_newview",enabledText:"Create New View",disableOnClick:false,onclick:function(){a.CswDialog("AddViewDialog",{onAddView:function(C){z=g(b,C)},onClose:function(){ga.CswButton("enable")},makeVisibilitySelect:o})}});s=v.CswWizard("div",CswViewEditor_WizardSteps.step2.step).CswTable({ID:l.ID+"_tbl2",FirstCellRightAlign:true});s.CswTable("cell",
1,1).append("View Name:");var ja=s.CswTable("cell",1,2).CswInput("init",{ID:l.ID+"_viewname",type:CswInput_Types.text});s.CswTable("cell",2,1).append("Category:");var sa=s.CswTable("cell",2,2).CswInput("init",{ID:l.ID+"_category",type:CswInput_Types.text}),ea;if(l.startingStep===1)ea=o(s,3,"View Visibility:");s.CswTable("cell",4,1).append("For Mobile:");var ra=s.CswTable("cell",4,2).CswInput("init",{ID:l.ID+"_formobile",type:CswInput_Types.checkbox});s.CswTable("cell",5,1).append("Display Mode:");
var wa=s.CswTable("cell",5,2).append('<span id="'+l.ID+'_displaymode"></span>'),Z=s.CswTable("cell",6,1).append("Grid Width (in characters):"),ia=s.CswTable("cell",6,2);ia.CswNumberTextBox("init",{ID:l.ID+"_gridwidth",Value:"",MinValue:"1",MaxValue:"",Precision:"0",onchange:function(){}});s=v.CswWizard("div",CswViewEditor_WizardSteps.step3.step);s.append("Add relationships from the select boxes below:<br/><br/>");var ka=a("<div />").appendTo(s);s=v.CswWizard("div",CswViewEditor_WizardSteps.step4.step);
s.append("Add properties from the select boxes below:<br/><br/>");var ta=a("<div />").appendTo(s);s=v.CswWizard("div",CswViewEditor_WizardSteps.step5.step);s.append("Add filters by selecting properties from the tree:<br/><br/>");var la=a("<div />").appendTo(s);s=v.CswWizard("div",CswViewEditor_WizardSteps.step6.step);s.append("Select what you want to edit from the tree:<br/><br/>");var ma=s.CswTable({ID:l.ID+"_6_tbl"}),O;return d}})(jQuery);var ViewBuilder_CssClasses={subfield_select:{name:"csw_viewbuilder_subfield_select"},filter_select:{name:"csw_viewbuilder_filter_select"},default_filter:{name:"csw_viewbuilder_default_filter"},filter_value:{name:"csw_viewbuilder_filter_value"},metadatatype_static:{name:"csw_viewbuilder_metadatatype_static"}};
...
b({AddWelcomeItemUrl:h.AddWelcomeItemUrl,type:i.val(),welcomepkval:t,nodetypeid:l.CswNodeTypeSelect("value"),text:s.val(),iconfilename:A.val(),onSuccess:h.onAdd,onError:function(){w.CswButton("enable")}})}});f.CswTable("cell",7,2).append(w);A.change(function(){v.CswAttrDom("src","Images/biggerbuttons/"+A.val())});i.change(function(){var t={$typeselect:i,$viewselect_label:q,$viewselect:o,$searchviewselect:k,$ntselect_label:m,$ntselect:l,$buttonsel_label:x,$buttonsel:A,$buttonimg:v},z={$typeselect:"",
...
g+" does not exist on CswWizard")}})(jQuery);var loginUrl="NewMain.html",timeout="";function getTimeout(){return timeout}
function CswAjaxJSON(a){var d={url:"",data:{},onloginfail:function(){_finishLogout()},success:function(){},error:function(){}};a&&$.extend(d,a);$.ajax({type:"POST",url:d.url,dataType:"json",contentType:"application/json; charset=utf-8",data:JSON.stringify(d.data),success:function(b,e,g){var f=$.parseJSON(b.d);if(f.error!==undefined){_handleAjaxError(g,{message:f.error.message,detail:f.error.detail},"");d.error()}else{b=tryParseString(f.AuthenticationStatus,"Unknown");timeout=tryParseString(f.timeout,
"");_handleAuthenticationStatus({status:b,success:function(){d.success(f)},failure:d.onloginfail,usernodeid:f.nodeid,usernodekey:f.cswnbtnodekey,passwordpropid:f.passwordpropid})}},error:function(b,e,g){_handleAjaxError(b,{message:"A Webservices Error Occurred",detail:e},g);d.error()}})}
function CswAjaxXml(a){var d={url:"",data:{},stringify:false,onloginfail:function(){_finishLogout()},success:function(){},error:function(){}};a&&$.extend(d,a);isNullOrEmpty(d.url)||$.ajax({type:"POST",url:d.url,dataType:"xml",data:$.param(d.data),success:function(b,e,g){var f=$(g.responseXML).children().first();if(f.first().get(0).nodeName==="error"){_handleAjaxError(g,{message:f.CswAttrXml("message"),detail:f.CswAttrXml("detail")},"");d.error()}else{b=tryParseString(f.CswAttrXml("authenticationstatus"),
"Unknown");timeout=f.CswAttrXml("timeout");_handleAuthenticationStatus({status:b,success:function(){d.success(f)},failure:d.onloginfail,usernodeid:tryParseString(f.CswAttrXml("nodeid"),""),usernodekey:tryParseString(f.CswAttrXml("cswnbtnodekey"),""),passwordpropid:tryParseString(f.CswAttrXml("passwordpropid"),"")})}},error:function(b,e,g){_handleAjaxError(b,{message:"A Webservices Error Occurred",detail:e},g);d.error()}})}
...
function Logout(a){var d={DeauthenticateUrl:"/NbtWebApp/wsNBT.asmx/deauthenticate",onDeauthenticate:function(){}};a&&$.extend(d,a);CswAjaxJSON({url:d.DeauthenticateUrl,data:{},success:function(){_finishLogout();d.onDeauthenticate()}})}function _finishLogout(){$.CswCookie("clearAll");window.location=loginUrl}
...
else a.error("Method "+b+" does not exist on CswFieldTypeLogical")}})(jQuery);(function(a){var d={init:function(b){var e=a(this);e.contents().remove();var g=b.$propxml.children("logicalsetxml");e=a("<div />").appendTo(e);var f=[],h=0;g.find("item").first().children("column").each(function(){var j=a(this).CswAttrXml("field");if(j!=="name"&&j!="key"){f[h]=j;h++}});var i=[],q=0;g.find("item").each(function(){for(var j=a(this),o=[],k=0,m=0;m<f.length;m++){var l=j.children('column[field="'+f[m]+'"]').CswAttrXml("value");o[k]=l==="True";k++}j={label:j.children('column[field="name"]').CswAttrXml("value"),
...
g.CswTable("cell",1,1).append("Answer");h=h.split(",");var m=a('<select id="'+e.ID+'_ans" />').appendTo(g.CswTable("cell",1,2)).change(function(){d(i,m,s,x);e.onchange()});o=a('<option value=""></option>').appendTo(m);for(k=0;k<h.length;k++){var l=h[k];o=a('<option value="'+l+'">'+l+"</option>").appendTo(m);l===f&&o.CswAttrDom("selected","true")}var s=g.CswTable("cell",2,1).append("Corrective Action"),x=a('<textarea id="'+e.ID+'_cor" />').appendTo(g.CswTable("cell",2,2)).text(j).change(function(){d(i,
m,s,x);e.onchange()});g.CswTable("cell",3,1).append("Comments");a('<textarea id="'+e.ID+'_com" />').appendTo(g.CswTable("cell",3,2)).text(q).change(e.onchange);d(i,m,s,x)}},save:function(e){var g=e.$propdiv.find("#"+e.ID+"_ans").val(),f=e.$propdiv.find("#"+e.ID+"_cor").val(),h=e.$propdiv.find("#"+e.ID+"_com").val();e.$propxml.children("answer").text(g);e.$propxml.children("correctiveaction").text(f);e.$propxml.children("comments").text(h)}};a.fn.CswFieldTypeQuestion=function(e){if(b[e])return b[e].apply(this,
...
f.ID+'_monthly_rate"/>').appendTo(m).change(f.onchange),s=1;s<=12;s++)l.append('<option value="'+s+'">'+s+"</option>");m.append(" Month(s)<br/>");var x=m.CswInput("init",{ID:f.ID+"_monthly_by_date",name:f.ID+"_monthly",type:CswInput_Types.radio,onChange:f.onchange,value:"MonthlyByDate"});m.append("On Day of Month:&nbsp;");var A=a('<select id="'+f.ID+'_monthly_date" />').appendTo(m).change(f.onchange);for(s=1;s<=31;s++)A.append('<option value="'+s+'">'+s+"</option>");m.append("<br/>");var v=m.CswInput("init",
{ID:f.ID+"_monthly_by_day",name:f.ID+"_monthly",type:CswInput_Types.radio,onChange:f.onchange,value:"MonthlyByWeekAndDay"});m.append("Every&nbsp;");var w=a('<select id="'+f.ID+'_monthly_week" />').appendTo(m);w.append('<option value="1">First:</option>');w.append('<option value="2">Second:</option>');w.append('<option value="3">Third:</option>');w.append('<option value="4">Fourth:</option>');m.append("<br/>");d(m,f.ID+"_monthly_day",f.onchange,true);m.append("<br/>Starting On:&nbsp;");var t=a('<select id="'+
f.ID+'_monthly_startMonth" />').appendTo(m).change(f.onchange);for(s=1;s<=12;s++)t.append('<option value="'+s+'">'+s+"</option>");m.append("/");var z=a('<select id="'+f.ID+'_monthly_startYear" />').appendTo(m).change(f.onchange),P=(new Date).getFullYear();for(s=P-10;s<=P+10;s++)z.append('<option value="'+s+'">'+s+"</option>");z.val(P);var K=a("<div />").appendTo(o).addClass("CswFieldTypeTimeInterval_Div");K.append("Every Year, Starting On:<br/>");o=K.CswInput("init",{ID:f.ID+"_yearly_sd",type:CswInput_Types.text,
...
A.val(s.children("monthlydate").text());t.val(s.children("startingmonth").text());z.val(s.children("startingyear").text());break;case "MonthlyByWeekAndDay":i.CswAttrDom("checked","true");k.hide();m.show();K.hide();v.CswAttrDom("checked","true");w.val(s.children("monthlyweek").text());b(f.ID+"_monthly_day",s.children("monthlyday").text());t.val(s.children("startingmonth").text());z.val(s.children("startingyear").text());break;case "YearlyByDate":j.CswAttrDom("checked","true");k.hide();m.show();K.hide();
