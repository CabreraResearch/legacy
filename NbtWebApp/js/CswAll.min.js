...
h++})}});g(d,600,400)},SearchDialog:function(){},FileUploadDialog:function(d){var f={url:"",params:{},onSuccess:function(){}};d&&a.extend(f,d);var e=a("<div></div>");new qq.FileUploader({element:e.get(0),action:f.url,params:f.params,debug:false,onComplete:function(){e.dialog("close");f.onSuccess()}});e.CswButton({ID:"fileupload_cancel",enabledText:"Cancel",disabledText:"Canceling",onclick:function(){e.dialog("close")}});g(e,400,300)},ShowLicenseDialog:function(d){var f={GetLicenseUrl:"/NbtWebApp/wsNBT.asmx/getLicense",
AcceptLicenseUrl:"/NbtWebApp/wsNBT.asmx/acceptLicense",onAccept:function(){},onDecline:function(){}};d&&a.extend(f,d);var e=a('<div align="center"></div>');e.append("Service Level Agreement<br/>");var h=a('<textarea id="license" disabled="true" rows="30" cols="80"></textarea>').appendTo(e);e.append("<br/>");CswAjaxJSON({url:f.GetLicenseUrl,success:function(j){h.text(j.license)}});e.CswButton({ID:"license_accept",enabledText:"I Accept",disabledText:"Accepting...",onclick:function(){CswAjaxJSON({url:f.AcceptLicenseUrl,
success:function(){e.dialog("close");f.onAccept()}})}});e.CswButton({ID:"license_decline",enabledText:"I Decline",disabledText:"Declining...",onclick:function(){e.dialog("close");f.onDecline()}});g(e,800,600)},OpenDialog:function(d,f){var e=a('<div id="'+d+'"></div>');e.load(f,{},function(){e.appendTo("body").dialog()})},CloseDialog:function(d){a("#"+d).dialog("close").remove()}};a.CswDialog=function(d){if(b[d])return b[d].apply(this,Array.prototype.slice.call(arguments,1));else if(typeof d==="object"||
...
var G=k.CswTable("findCell",'[row="'+q+'"][column="'+s+'"]');G.removeClass("CswLayoutTable_dragcell");var B=i(k,q,s,t,u,y);if(B.length>0){k.trigger(k.attr("id")+"CswLayoutTable_onSwap",{table:k,cellset:b(k,q,s),swapcellset:b(k,B.first().attr("row"),B.first().attr("column")),row:q,column:s,swaprow:B.first().attr("row"),swapcolumn:B.first().attr("column")});var N=a("<div />");for(r=1;r<=t;r++)for(c=1;c<=u;c++){var O=G.filter('[cellsetrow="'+r+'"][cellsetcolumn="'+c+'"]'),K=B.filter('[cellsetrow="'+
r+'"][cellsetcolumn="'+c+'"]');O.contents().appendTo(N);K.contents().appendTo(O);N.contents().appendTo(K)}}}}).hover(function(){var v=a(this);f(k)&&v.addClass("CswLayoutTable_hover");d(k)&&k.CswTable("findCell",'[row="'+v.attr("row")+'"][column="'+v.attr("column")+'"]').addClass("CswLayoutTable_remove")},function(){var v=a(this);f(k)&&v.removeClass("CswLayoutTable_hover");d(k)&&k.CswTable("findCell",'[row="'+v.attr("row")+'"][column="'+v.attr("column")+'"]').removeClass("CswLayoutTable_remove")})}
...
function j(l,p,o,t,u,q,s,v,y,G,B){a.CswFieldTypeFactory("save",l);CswAjaxXml({url:p,data:"EditMode="+o+"&SafeNodeKey="+t+"&PropId="+u+"&NodeTypeId="+q+"&NewPropXml="+xmlToString(s),success:function(N){h(v,N.children().first(),y,G,B)}})}function n(l,p,o){if(l.valid()){i(p,o);l={EditMode:m.EditMode,SafeNodeKey:m.cswnbtnodekey,NodeTypeId:m.nodetypeid,ViewId:a.CswCookie("get",CswCookieName.CurrentView.ViewId),NewPropsXml:xmlToString(o)};CswAjaxJSON({url:m.SavePropUrl,data:jsonToString(l),success:function(t){if(m.ShowCheckboxes){var u=
a("."+m.NodeCheckTreeId+"_check:checked"),q=a("."+m.ID+"_check:checked");if(u.length>0&&q.length>0){var s="{ SourceNodeKey: '"+m.cswnbtnodekey+"', CopyNodeIds: [",v=true;u.each(function(){var y=a(this).attr("nodeid");v||(s+=",");s+="'"+y+"'";v=false});s+="], PropIds: [";v=true;q.each(function(){var y=a(this).attr("propid");v||(s+=",");s+="'"+y+"'";v=false});s+="] }";CswAjaxJSON({url:m.CopyPropValuesUrl,data:s})}}m.onSave(t.nodeid,t.cswnbtnodekey)}})}}function i(l,p){p.children().each(function(){var o=
{$propxml:a(this),$propdiv:"",$propCell:"",fieldtype:"",nodeid:m.nodeid,cswnbtnodekey:m.cswnbtnodekey};o.fieldtype=o.$propxml.attr("fieldtype");var t=l.CswLayoutTable("cellset",o.$propxml.attr("displayrow"),o.$propxml.attr("displaycol"));o.$propcell=t[1][2];o.$propdiv=o.$propcell.children("div").first();a.CswFieldTypeFactory("save",o);if(o.$propxml.attr("hassubprops")=="true"){t=o.$propxml.children("subprops");if(t.length>0&&t.children('[display != "false"]').length>0){o=o.$propcell.children("#"+
o.$propxml.attr("id")+"_subproptable").first();i(o,t)}}})}var m={ID:"",TabsUrl:"/NbtWebApp/wsNBT.asmx/getTabs",SinglePropUrl:"/NbtWebApp/wsNBT.asmx/getSingleProp",PropsUrl:"/NbtWebApp/wsNBT.asmx/getProps",MovePropUrl:"/NbtWebApp/wsNBT.asmx/moveProp",SavePropUrl:"/NbtWebApp/wsNBT.asmx/saveProps",CopyPropValuesUrl:"/NbtWebApp/wsNBT.asmx/copyPropValues",nodeid:"",relatednodeid:"",tabid:"",cswnbtnodekey:"",nodetypeid:"",filterToPropId:"",title:"",EditMode:"Edit",onSave:function(){},onBeforeTabSelect:function(){return true},
onTabSelect:function(){},onPropertyChange:function(){},ShowCheckboxes:false,NodeCheckTreeId:""};g&&a.extend(m,g);g=a(this);var k=a('<div id="'+m.ID+'_tabdiv" />').appendTo(g);(function(){CswAjaxXml({url:m.TabsUrl,data:"EditMode="+m.EditMode+"&SafeNodeKey="+m.cswnbtnodekey+"&NodeTypeId="+m.nodetypeid,success:function(l){k.contents().remove();var p=[],o=0,t=0;l.children().each(function(){$tab=a(this);if(m.EditMode=="PrintReport"||p.length==0)p[p.length]=a("<div><ul></ul></div>").appendTo(k);var v=p[p.length-
1];v.children("ul").append('<li><a href="#'+$tab.attr("id")+'">'+$tab.attr("name")+"</a></li>");v.append('<div id="'+$tab.attr("id")+'"><form id="'+$tab.attr("id")+'_form" /></div>');if($tab.attr("id")==m.tabid)o=t;t++});for(var u in p){var q=p[u];q.tabs({selected:o,select:function(v,y){if(m.onBeforeTabSelect(B)){var G=a(q.children("div")[y.index]),B=G.attr("id");b(G,B);m.onTabSelect(B)}else return false}});l=a(q.children("div")[q.tabs("option","selected")]);var s=l.attr("id");b(l,s);m.onTabSelect(s)}}})})(m);
if(m.EditMode!="PrintReport"){g=a('<div id="'+m.ID+'_linkdiv" align="right"/>').appendTo(g);a('<a href="#">As Report</a>').appendTo(g).click(function(){openPopup("NewNodeReport.html?nodeid="+m.nodeid+"&cswnbtnodekey="+m.cswnbtnodekey,600,800)})}return this}})(jQuery);(function(a){function g(d,f){return jsTreeGetSelected(d).$item.attr("rel")==f.attr("rel")}var b={init:function(d){var f={ID:"",ViewTreeUrl:"/NbtWebApp/wsNBT.asmx/getTreeOfView",NodeTreeUrl:"/NbtWebApp/wsNBT.asmx/getTreeOfNode",viewid:"",viewmode:"",nodeid:"",cswnbtnodekey:"",IncludeNodeRequired:false,UsePaging:true,onSelectNode:function(){},onViewChange:function(){},SelectFirstChild:true,ShowCheckboxes:false};d&&a.extend(f,d);var e=f.ID+"_",h=a('<div id="'+e+'" class="treediv" />').appendTo(a(this)),
...
q.attr("id").substring(e.length),v=q.children("content").children("name").text(),y='<li id="'+q.attr("id")+'" ';y+='    rel="'+q.attr("rel")+'" ';y+='    species="'+q.attr("species")+'" ';y+='    class="jstree-'+q.attr("state")+'" ';if(q.attr("cswnbtnodekey")!=undefined)y+='    cswnbtnodekey="'+q.attr("cswnbtnodekey").replace(/"/g,"&quot;")+'"';y+=">";if(f.ShowCheckboxes)y+='  <input type="checkbox" class="'+e+'check" id="check_'+s+'" rel="'+q.attr("rel")+'" nodeid="'+s+'" nodename="'+v+'"></input>';
y+='  <a href="#">'+v+"</a>";if(q.children("item").length>0){y+="<ul>";q.children("item").each(function(){y+=t(a(this))});y+="</ul>"}y+="</li>";return y},u="<ul>";n.children().each(function(){u+=t(a(this))});u+="</ul>";h.jstree({html_data:{data:u,ajax:{type:"POST",url:j,dataType:"xml",data:function(q){q=q.attr("cswnbtnodekey");return"UsePaging="+f.UsePaging+"&ViewNum="+f.viewid+"&IDPrefix="+e+"&IsFirstLoad=false&ParentNodeKey="+q+"&IncludeNodeRequired=false&IncludeNodeKey="},success:function(q,s,
v){var y="";a(v.responseXML).children().first().children().each(function(){y+=t(a(this))});return y}}},ui:{select_limit:1,initially_select:i},themes:k,core:{initially_open:p},types:{types:l,max_children:-2,max_depth:-2},plugins:m}).bind("load_node.jstree",function(){a("."+e+"check").unbind("click");a("."+e+"check").click(function(){return g(h,a(this))})}).bind("select_node.jstree",function(q,s){var v=jsTreeGetSelected(h),y={nodeid:v.id,nodename:v.text,iconurl:v.iconurl,cswnbtnodekey:v.$item.attr("cswnbtnodekey"),
nodespecies:v.$item.attr("species"),viewid:f.viewid};if(y.nodespecies=="More"){v="";var G=s.inst._get_parent(s.rslt.obj);if(G!=-1)v=G.attr("cswnbtnodekey");if(v==undefined)v="";CswAjaxXml({url:j,data:"UsePaging="+f.UsePaging+"&ViewNum="+f.viewid+"&IDPrefix="+e+"&IsFirstLoad=false&ParentNodeKey="+v+"&IncludeNodeRequired=false&IncludeNodeKey="+y.cswnbtnodekey,success:function(B){function N(){K.length>0&&h.jstree("create","#"+O,"after",{attr:{id:K.attr("id"),rel:K.attr("rel"),cswnbtnodekey:K.attr("cswnbtnodekey"),
species:K.attr("species")},data:K.children("content").children("name").text(),state:K.attr("state")},function(){O==K.attr("id")&&h.jstree("remove","#"+e+y.nodeid+'[species="More"]');O=K.attr("id");K=K.next();N()},true,true)}var O=e+y.nodeid,K=B.children().first();N()}})}else{a("."+e+"check").attr("checked","");f.onSelectNode(y)}});a("."+e+"check").click(function(){return g(h,a(this))});h.find("li").each(function(){var q=a(this);q.children("ins").attr("id",q.attr("id")+"_expand")})}else h.append("No Results")}});
...
ViewBuilder_CssClasses.subfield_select.name).each(function(){a(this).hide()});a("."+ViewBuilder_CssClasses.filter_select.name).each(function(){a(this).hide()});a("."+ViewBuilder_CssClasses.default_filter.name).each(function(){a(this).show()});l.$link.text("Advanced");l.advancedIsHidden=false}return l.advancedIsHidden}function d(){for(var k=2;k<=i.propsCount;){var l=i.$searchTable.CswTable("cell",k,1).attr({align:"right"}),p=a("<span>&nbsp;and&nbsp;</span>");l.append(p);k++}var o=1;i.$propsXml.children("property").each(function(){var t=
a(this),u=i.$searchTable.CswTable("cell",o,2),q=makeId({ID:"viewbuilderpropid",suffix:t.attr("viewbuilderpropid"),prefix:i.ID});u.CswDOM("span",{ID:q,value:t.attr("metadatatypename"),cssclass:ViewBuilder_CssClasses.metadatatype_static.name}).attr("relatedidtype",t.attr("relatedidtype"));i.selectedSubfieldVal="";i.selectedFilterVal="";u=t.attr("filtarbitraryid");i.$searchTable.CswViewPropFilter("init",{ID:i.ID,propRow:o,firstColumn:3,includePropertyName:true,$propsXml:t,filtarbitraryid:u,advancedIsHidden:i.advancedIsHidden});
o++});i.bottomRow=o;i.bottomCell=1;i.searchtype="viewsearch";h()}function f(){var k=i.$searchTable.CswTable("cell",1,2).empty(),l=makeId({ID:"nodetype_select",prefix:i.ID});l=a(xmlToString(i.$nodeTypesXml.children("select"))).attr("id",l).attr("name",l).attr("class",CswSearch_CssClasses.nodetype_select.name).change(function(){var p=a(this);p={nodetypeorobjectclassid:p.val(),relatedidtype:p.find(":selected").attr("title"),cswnbtnodekey:"",$parent:i.$searchTable,$nodeTypesSelect:p};a.extend(i,p);e()});
i.$nodeTypesSelect=l;i.relatedidtype=l.find(":selected").attr("title");i.nodetypeorobjectclassid!==""&&l.val(i.nodetypeorobjectclassid).attr("selected",true);k.append(l);k=i.$searchTable.CswTable("cell",1,3).empty();l=makeId({ID:"property_select",prefix:i.ID});l=a(xmlToString(i.$propsXml.children("properties").children("select"))).attr("id",l).attr("name",l).attr("class",CswSearch_CssClasses.property_select.name).change(function(){var p=a(this).val(),o={propertyid:p,selectedSubfieldVal:"",selectedFilterVal:"",
$selectedPropXml:i.$propsXml.children("propertyfilters").children("property[viewbuilderpropid="+p+"]")};a.extend(i,o);i.$searchTable.CswViewPropFilter("init",{ID:i.ID,propRow:1,firstColumn:3,includePropertyName:false,$propsXml:i.$selectedPropXml,viewbuilderpropid:p,advancedIsHidden:i.advancedIsHidden})});i.propertyid!==""&&l.val(i.propertyid).attr("selected",true);k.append(l);i.propertyid=l.find(":selected").val();i.$selectedPropXml=i.$propsXml.children("propertyfilters").children("property[viewbuilderpropid="+
...
k;f()}})}function h(){var k=i.$searchTable,l=i.bottomCell,p=l+1,o=i.bottomRow;if(i.searchtype==="nodetypesearch"){k=i.$searchTable.CswTable("cell",i.bottomRow,i.bottomCell).empty();p=makeId({prefix:i.ID,ID:"split_cell_table"});k=k.CswTable("init",{ID:p,cellpadding:1,cellspacing:1,cellalign:"left",align:"left"});l=1;p=2;o=1}l=k.CswTable("cell",o,l).empty();var t=makeId({ID:"clear_button",prefix:i.ID});l.CswButton({ID:t,enabledText:"Clear",disabledText:"Clear",disableOnClick:false,onclick:function(){m.empty();
var q=makeId({prefix:i.ID,ID:"search_tbl"});i.$searchTable=m.CswTable("init",{ID:q,cellpadding:1,cellspacing:1,cellalign:"center",align:"center"});switch(i.searchtype){case "nodetypesearch":f();case "viewsearch":d()}}});k=k.CswTable("cell",o,p).empty();p=makeId({ID:"advanced_options",prefix:i.ID});var u=k.CswDOM("link",{ID:p,href:"#advanced",value:"Advanced"}).click(function(){i.advancedIsHidden=b({$link:u,advancedIsHidden:i.advancedIsHidden})});k=i.$searchTable.CswTable("cell",i.bottomRow,i.searchBtnCell).attr({align:"right"}).empty();
p=makeId({ID:"search_button",prefix:i.ID});k.CswButton({ID:p,enabledText:"Search",disabledText:"Searching",enableAfterClick:true,onclick:function(){n()}})}function j(){CswAjaxXml({url:i.getClientSearchXmlUrl,data:"ViewIdNum="+i.viewid+"&SelectedNodeTypeIdNum="+i.nodetypeorobjectclassid+"&IdPrefix="+i.ID+"&NodeKey="+i.cswnbtnodekey,success:function(k){m.empty();i.searchtype=k.attr("searchtype");var l=makeId({prefix:i.ID,ID:"search_tbl"});i.$searchTable=m.CswTable("init",{ID:l,cellpadding:1,cellspacing:1,
cellalign:"left",align:"center"});i.$searchTable.css("background-color","0099FF");i.$searchTable.attr("frame","border");switch(i.searchtype){case "nodetypesearch":i.$nodeTypesXml=k.children("nodetypes");i.$propsXml=k.children("viewbuilderprops");f();break;case "viewsearch":i.$propsXml=k.children("properties");i.propsCount=k.children("properties").children("property").size();d()}}})}function n(){var k,l=[],p;switch(i.searchtype){case "nodetypesearch":p=i.doNodeSearchUrl;var o=i.$nodeTypesSelect.val();
i.relatedidtype=i.$nodeTypesSelect.find(":selected").attr("title");a("."+CswSearch_CssClasses.property_select.name).each(function(){var t=a(this),u=t.text(),q=t.val();u=i.$propsXml.children("propertyfilters").children('property[propname="'+u+'"][viewbuilderpropid="'+q+'"]').attr("fieldtype");t=t.CswViewPropFilter("getFilterJson",{nodetypeorobjectclassid:o,relatedidtype:i.relatedidtype,fieldtype:u,ID:i.ID,$parent:i.$searchTable,viewbuilderpropid:q,advancedIsHidden:i.advancedIsHidden});l.push(t)});
k={viewbuilderprops:l};break;case "viewsearch":p=i.doViewSearchUrl;i.$propsXml.children("property").each(function(){var t=a(this),u=t.attr("filtarbitraryid");t=t.CswViewPropFilter("getFilterJson",{ID:i.ID,$parent:i.$searchTable,filterarbitraryid:u});l.push(t)});k={viewprops:l,viewid:i.viewid}}k&&CswAjaxJSON({url:p,data:'{SearchJson: "'+jsonToString(k)+'"}',success:function(t){i.viewid=t.sessionviewid;i.searchtype="viewsearch";j();i.onSearchSubmit({viewid:t.sessionviewid,viewmode:t.viewmode})}})}var i=
{$parent:"",$searchTable:"",getNewPropsUrl:"/NbtWebApp/wsNBT.asmx/getNodeTypeSearchProps",doViewSearchUrl:"/NbtWebApp/wsNBT.asmx/doViewSearch",doNodeSearchUrl:"/NbtWebApp/wsNBT.asmx/doNodeTypeSearch",getSearchableViewsUrl:"/NbtWebApp/wsNBT.asmx/getSearchableViews",getClientSearchXmlUrl:"/NbtWebApp/wsNBT.asmx/getClientSearchXml",viewid:"",nodetypeorobjectclassid:"",propertyid:"",cswnbtnodekey:"",relatedidtype:"",ID:"",propsCount:1,advancedIsHidden:true,$propsXml:"",$selectedPropXml:"",$nodeTypesXml:"",
$nodeTypesSelect:"",onSearchSubmit:function(){},selectedPropVal:"",selectedSubfieldVal:"",selectedFilterVal:"",bottomRow:2,bottomCell:1,propRow:"",clearBtnCell:1,searchBtnCell:6,searchtype:""};g&&a.extend(i,g);g=a(this);i.$searchTable=g.CswDOM("div",{ID:i.ID});var m=i.$searchTable.CswDOM("span").CswDOM("div",{ID:"search_criteria_div",prefix:i.ID});j();return i.$searchTable}})(jQuery);(function(a){a.fn.CswTable=function(g){function b(h){return h.children("tbody").children("tr").length}function d(h){h=h.children("tbody").children("tr");for(var j=0,n=0;n<h.length;n++){var i=a(h[n]).children("td");if(i.length>j)j=i.length}return j}function f(h,j,n){var i=null;if(h.length>0&&j!=undefined&&j!=""&&n!=undefined&&n!=""){if(j<=0){log("error: row must be greater than 1, got: "+j);j=1}if(n<=0){log("error: col must be greater than 1, got: "+n);n=1}for(;j>h.children("tbody").children("tr").length;)h.append("<tr></tr>");
...
{viewxml:xmlToString(D),proparbitraryid:w.attr("proparbid"),filtarbitraryid:"",viewbuilderpropid:"",ID:i.ID,propRow:1,firstColumn:1,includePropertyName:false,selectedSubfieldVal:"",selectedFilterVal:"",autoFocusInput:false});A.CswTable("cell",1,5).CswButton("init",{ID:"addfiltbtn",prefix:i.ID,enabledText:"Add",disabledText:"Adding",onclick:function(){var H=A.CswViewPropFilter("getFilterJson",{ID:i.ID,$parent:w,proparbitraryid:w.attr("proparbid")});A.CswViewPropFilter("makeFilter",{viewxml:xmlToString(D),
...
i.ID+'_adcb" />').appendTo(A.CswTable("cell",1,2));w.attr("allowdelete").toLowerCase()=="true"&&H.attr("checked","true");A.CswTable("cell",2,1).append("Group By");var I=a('<select id="'+i.ID+'_gbs" />').appendTo(A.CswTable("cell",2,2));CswAjaxXml({url:i.PropNamesUrl,data:"Type="+w.attr("secondtype")+"&Id="+w.attr("secondid"),success:function(J){I.empty();a('<option value="">[None]</option>').appendTo(I);J.children().each(function(){var M=a(this),P=a('<option value="'+M.attr("propid")+'">'+M.attr("propname")+
"</option>").appendTo(I).data("propxml",M);w.attr("groupbypropid")==M.attr("propid")&&w.attr("groupbyproptype")==M.attr("proptype")&&w.attr("groupbypropname")==M.attr("propname")&&P.attr("selected","true")})}});var L;if(F=="Tree"){A.CswTable("cell",3,1).append("Show In Tree");L=a('<input type="checkbox" id="'+i.ID+'_stcb" />').appendTo(A.CswTable("cell",3,2));w.attr("showintree").toLowerCase()=="true"&&L.attr("checked","true")}A.CswTable("cell",4,2).CswButton({ID:i.ID+"_saverel",enabledText:"Apply",
...
var G=a('<input type="textbox" id="'+i.ID+'_category" />').appendTo(k.CswTable("cell",2,2)),B=h(k,3,"View Visibility:");k.CswTable("cell",4,1).append("For Mobile:");var N=a('<input type="checkbox" id="'+i.ID+'_formobile" />').appendTo(k.CswTable("cell",4,2));k.CswTable("cell",5,1).append("Display Mode:");var O=k.CswTable("cell",5,2).append('<span id="'+i.ID+'_displaymode"></span>'),K=k.CswTable("cell",6,1).append("Grid Width (in characters):"),Q=k.CswTable("cell",6,2);Q.CswNumberTextBox("init",{ID:i.ID+
...
e.ID!==""&&e.ID!==undefined?makeId({ID:d,prefix:e.ID}):d}var b={init:function(d){function f(n){e.$propsXml.attr("viewbuilderpropid");var i=e.$propsXml.attr("propname");if(e.includePropertyName){var m=h.CswTable("cell",e.propRow,e.firstColumn).empty(),k=g(i,n);m.CswDOM("span",{ID:k,value:i})}m=e.$propsXml.attr("fieldtype");var l=e.$propsXml.children("defaultsubfield").attr("filter");k=h.CswTable("cell",e.propRow,e.firstColumn+1).empty();var p=g("default_filter",n);l=k.CswDOM("span",{ID:p,value:l,cssclass:ViewBuilder_CssClasses.default_filter.name}).attr({align:"center"});
e.advancedIsHidden||l.hide();l=g("subfield_select",n);var o=a(xmlToString(e.$propsXml.children("subfields").children("select"))).attr("id",l).attr("name",l).attr("class",ViewBuilder_CssClasses.subfield_select.name).change(function(){var u={selectedSubfieldVal:a(this).val(),selectedFilterVal:""};a.extend(e,u);f(n)});e.selectedSubfieldVal!==""&&o.val(e.selectedSubfieldVal).attr("selected",true);k.append(o);e.advancedIsHidden&&o.hide();p=o.find(":selected").val();k=o.find(":selected").attr("defaultvalue");
l=h.CswTable("cell",e.propRow,e.firstColumn+2).empty();var t=g("filter_select",n);p=a(xmlToString(e.$propsXml.children("propertyfilters").children("subfield[column="+p+"]").children("select"))).attr("id",t).attr("name",t).attr("class",ViewBuilder_CssClasses.filter_select.name).change(function(){var u=a(this);u={selectedSubfieldVal:o.val(),selectedFilterVal:u.val()};a.extend(e,u);f(n)});e.selectedFilterVal!==""&&p.val(e.selectedFilterVal).attr("selected",true);l.append(p);e.advancedIsHidden&&p.hide();
l=h.CswTable("cell",e.propRow,e.firstColumn+3).empty();p=g("propfilter_input",n);if(m==="List")l.append(a(xmlToString(e.$propsXml.children("filtersoptions").children("select"))));else if(m==="Logical")l.CswTristateCheckBox("init",{ID:p,Checked:k});else{m={value:k,placeholder:""};if(m.value===""||m.value===undefined){e.placeholder=i;if(e.placeholder!==o.find(":selected").text())e.placeholder+="'s "+o.find(":selected").text()}l.CswInput("init",{ID:p,type:CswInput_Types.text,cssclass:ViewBuilder_CssClasses.filter_value.name,
...
"")a=$('<li class="headermenu_dialog">'+g.$itemxml.attr("text")+"</li>").appendTo(g.$ul).click(function(){OpenDialog(g.$itemxml.attr("text"),g.$itemxml.attr("popup"))});else if(g.$itemxml.attr("action")!=undefined&&g.$itemxml.attr("action")!=""){a=$('<li><a href="#">'+g.$itemxml.attr("text")+"</a></li>").appendTo(g.$ul);var b=a.children("a");switch(g.$itemxml.attr("action")){case "About":b.click(function(){$.CswDialog("AboutDialog");return false});break;case "AddNode":b.click(function(){$.CswDialog("AddNodeDialog",
{nodetypeid:g.$itemxml.attr("nodetypeid"),relatednodeid:g.$itemxml.attr("relatednodeid"),onAddNode:g.onAlterNode});return false});break;case "DeleteNode":b.click(function(){$.CswDialog("DeleteNodeDialog",{nodename:g.$itemxml.attr("nodename"),nodeid:g.$itemxml.attr("nodeid"),onDeleteNode:g.onAlterNode,NodeCheckTreeId:g.NodeCheckTreeId,Multi:g.Multi});return false});break;case "editview":b.click(function(){g.onEditView(g.$itemxml.attr("viewid"));return false});break;case "CopyNode":b.click(function(){$.CswDialog("CopyNodeDialog",
{nodename:g.$itemxml.attr("nodename"),nodeid:g.$itemxml.attr("nodeid"),onCopyNode:g.onAlterNode});return false});break;case "Home":b.click(function(){GoHome();return false});break;case "Logout":b.click(function(){g.onLogout();return false});break;case "ViewSearch":b.click(function(){g.onSearch.onViewSearch()});break;case "GenericSearch":b.click(function(){g.onSearch.onGenericSearch()});break;case "multiedit":b.click(g.onMultiEdit)}}else a=$("<li>"+g.$itemxml.attr("text")+"</li>").appendTo(g.$ul);
return a}function openPopup(a,g,b){a=window.open(a,null,"height="+g+", width="+b+", status=no, resizable=yes, scrollbars=yes, toolbar=yes, location=no, menubar=yes");a.focus();return a}function validateTime(a){var g=true,b=/^(\d?\d):(\d\d)\s?([APap][Mm])?$/g.exec(a);if(b==null)g=false;else{a=parseInt(b[1]);b=parseInt(b[2]);if(a<0||a>=24||b<0||b>=60)g=false}return g}
function validateFloatMinValue(a,g){var b=parseFloat(a),d=parseFloat(g),f=true;if(d!=undefined)if(b==undefined||b<d)f=false;return f}function validateFloatMaxValue(a,g){var b=parseFloat(a),d=parseFloat(g),f=true;if(d!=undefined)if(b==undefined||b>d)f=false;return f}function validateFloatPrecision(a,g){var b=true,d;d=g>0?/^\-?\d*\.?\d*$/g:/^\-?\d*$/g;if(b&&!d.test(a))b=false;return b}function validateInteger(a){return/^\-?\d*$/g.test(a)}function startsWith(a,g){return a.substr(0,g.length)==g}
function getTimeString(a){var g="",b=a.getHours();a=a.getMinutes();if(a<10)a="0"+a;g=b%12+":"+a+" ";g+=b>11?"PM":"AM";return g}function makeId(a){var g={ID:"",prefix:"",suffix:"",Delimiter:"_"};a&&$.extend(g,a);a=g.ID;if(g.prefix!==""&&a!=="")a=g.prefix+g.Delimiter+a;if(g.suffix!==""&&a!=="")a+=g.Delimiter+g.suffix;return a}function iterate(a){var g,b;for(b in a)g=g+b+"="+a[b]+"<br><br>";a=window.open("","popup");a!=null?a.document.write(g):console.log("iterate() error: No popup!")}
function log(a){try{console.log(a)}catch(g){alert(a)}}if(typeof String.prototype.trim!=="function")String.prototype.trim=function(){return this.replace(/^\s+|\s+$/g,"")};(function(a){var g={init:function(b){var d=a(this);d.contents().remove();var f=b.$propxml.children("barcode").text().trim();if(b.ReadOnly)d.append(f);else{d=d.CswTable("init",{ID:b.ID+"_tbl"});var e=d.CswTable("cell",1,1);f=a('<input type="text" class="textinput" id="'+b.ID+'" name="'+b.ID+'" value="'+f+'" />"').appendTo(e).change(b.onchange);d=d.CswTable("cell",1,2);a("<div/>").appendTo(d).CswImageButton({ButtonType:CswImageButton_ButtonType.Print,AlternateText:"",ID:b.ID+"_print",onClick:function(){a.CswDialog("OpenDialog",
...
n=d.$propxml.children("comments").text().trim(),i=d.$propxml.children("correctiveaction").text().trim(),m=d.$propxml.children("dateanswered").text().trim(),k=d.$propxml.children("datecorrected").text().trim();d.$propxml.children("iscompliant").text().trim();if(d.ReadOnly){f.append("Answer: "+e);m!=""&&f.append(" ("+m+")");f.append("<br/>");f.append("Corrective Action: "+i);k!=""&&f.append(" ("+k+")");f.append("<br/>");f.append("Comments: "+n+"<br/>")}else{f=f.CswTable("init",{ID:d.ID+"_tbl",FirstCellRightAlign:true});
f.CswTable("cell",1,1).append("Answer");h=h.split(",");var l=a('<select id="'+d.ID+'_ans" />').appendTo(f.CswTable("cell",1,2)).change(function(){g(j,l,o,t);d.onchange()});m=a('<option value=""></option>').appendTo(l);for(k=0;k<h.length;k++){var p=h[k];m=a('<option value="'+p+'">'+p+"</option>").appendTo(l);p==e&&m.attr("selected","true")}var o=f.CswTable("cell",2,1).append("Corrective Action"),t=a('<textarea id="'+d.ID+'_cor" />').appendTo(f.CswTable("cell",2,2)).text(i).change(function(){g(j,l,
o,t);d.onchange()});f.CswTable("cell",3,1).append("Comments");a('<textarea id="'+d.ID+'_com" />').appendTo(f.CswTable("cell",3,2)).text(n).change(d.onchange);g(j,l,o,t)}},save:function(d){var f=d.$propdiv.find("#"+d.ID+"_ans").val(),e=d.$propdiv.find("#"+d.ID+"_cor").val(),h=d.$propdiv.find("#"+d.ID+"_com").val();d.$propxml.children("answer").text(f);d.$propxml.children("correctiveaction").text(e);d.$propxml.children("comments").text(h)}};a.fn.CswFieldTypeQuestion=function(d){if(b[d])return b[d].apply(this,
