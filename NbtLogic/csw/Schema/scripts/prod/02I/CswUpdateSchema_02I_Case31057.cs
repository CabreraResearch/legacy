...
namespace ChemSW.Nbt.Schema
{
    ...
    public class CswUpdateSchema_02I_Case31057: CswUpdateSchemaTo
    {
        ...
        public override void update()
        {
            ...
            string where = "where jctnodepropid in (select jnp.field_2, jnp.clobdata from jct_nodes_props jnp" +
                         " join nodetype_props ntp on jnp.nodetypepropid = ntp.nodetypepropid" +
                         "  where ntp.fieldtypeid = " + LinkFieldType.FieldTypeId + " and jnp.field_2 is not null)";

            CswTableUpdate linkTU = _CswNbtSchemaModTrnsctn.makeCswTableUpdate( "31057.UpdateLinkPropsHrefCol", "jct_nodes_props" );
            DataTable jct_nodes_props = linkTU.getTable( new CswCommaDelimitedString() { "clobdata,field_2" }, string.Empty, Int32.MinValue, where, false );

            foreach( DataRow Row in jct_nodes_props.Rows )
            {
                Row["clobdata"] = Row["field_2"];
                Row["field_2"] = string.Empty;
            }
