...
namespace ChemSW.Nbt.PropTypes
{
    ...
    public class CswNbtNodePropScientific : CswNbtNodeProp
    {
        ...
        public override void ToJSON( JObject ParentObject )
        {
            if( false == Double.IsNaN( Base ) )
            {
                ParentObject[_BaseSubField.ToXmlNodeName()] = Base.ToString();
            }
            if( Int32.MinValue != Exponent )
            {
                ParentObject[_ExponentSubField.ToXmlNodeName()] = Exponent.ToString();
            }
        }

        public override void ReadJSON( JObject JObject, Dictionary<Int32, Int32> NodeMap, Dictionary<Int32, Int32> NodeTypeMap )
        {
            Base = CswConvert.ToDouble( JObject[_BaseSubField.ToXmlNodeName()] );
            Exponent = CswConvert.ToInt32( JObject[_ExponentSubField.ToXmlNodeName()] );
        }
