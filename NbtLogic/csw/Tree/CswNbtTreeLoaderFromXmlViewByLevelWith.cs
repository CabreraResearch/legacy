...
{
    public class CswNbtTreeLoaderFromXmlViewByLevelWith : CswNbtTreeLoader
    {
        ...
        private void loadRelationshipRecursive( CswNbtViewRelationship Relationship, bool RequireViewPermissions )
        {
            ...
            foreach( DataRow NodesRow in NodesTable.Rows )
            {
                ...
                if( false == RequireViewPermissions || _CswNbtResources.Permit.canNode( CswNbtPermit.NodeTypePermission.View, ThisNodeType, ThisNodePk, _RunAsUser ) )
                {
        ...
        private string _addInnerNodeId( object nodeId )
        {
            return " select " + CswConvert.ToString( nodeId ) + " nodeid from dual union ";
        }
        ...
        private string _makeNodeSql( CswNbtViewRelationship Relationship, bool IsParentQuery )
        {
            ...
            string Where = "where t.enabled = '1' ";
            ...
            if( Relationship.PropId != Int32.MinValue )
            {
                ...
                {
                    JnpCount += 1;
                    From += @"            join jnp x1 on (j1.nodetypepropid = p.nodetypepropid ";
                    if( Relationship.PropType == NbtViewPropIdType.NodeTypePropId )
                    {
                        From += @"               and x1.firstpropversionid = " + Relationship.PropId;
                    }
                    else
                    {
                        From += @"               and x1.objectclasspropid = " + Relationship.PropId;
                    }
                    From += @"                and x1.thisnodeid = n.nodeid)";
                }
                else
                {
                    JnpCount += 1;
                    From += @"          join jnp x2 on (jnp.nodetypepropid = p.nodetypepropid ";
                    if( Relationship.PropType == NbtViewPropIdType.NodeTypePropId )
                    {
                        From += @"             and x2.firstpropversionid = " + Relationship.PropId;
                    }
                    else
                    {
                        From += @"              and x2.objectclasspropid = " + Relationship.PropId;
                    }
                    From += @"        and x2.thisnodeid = n.nodeid)";
                }
            } // if( Relationship.PropId != Int32.MinValue )
            ...
            if( false == IsParentQuery )
            {
                if( Relationship.GroupByPropId != Int32.MinValue )
                {
                    ...
                    WithGrp = @" gby as (select jnp.nodeid thisnodeid, p.nodetypepropid, p.objectclasspropid, " + GroupBySubField.Column + @"
                                       from jct_nodes_props jnp
                                       join nodetype_props p on jnp.nodetypepropid = p.nodetypepropid ) ";
                    ...
                    {
                        From += @" left outer join gby g1 on (g1.objectclasspropid = " + Relationship.GroupByPropId.ToString() + " and g1.nodeid = n.nodeid)";
                    }
                    else
                    {
                        From += @" left outer join gby g1 on (g1.nodetypepropid = " + Relationship.GroupByPropId.ToString() + " and g.nodeid = n.nodeid)";
                    }
                } // if( Relationship.GroupByPropId != Int32.MinValue )
                ...
                foreach( CswNbtViewProperty Prop in Relationship.Properties )
                {
                    if( Prop.SortBy )
                    {
                        ...
                        if( null != Prop.MetaDataProp )
                        {
                            ...
                            SrtCount += 1;
                            From += " left outer join jsort j" + sortAlias + " ";
                            ...
                            {
                                From += " j" + sortAlias + ".objectclasspropid = " + Prop.ObjectClassPropId + ") ";
                            }
